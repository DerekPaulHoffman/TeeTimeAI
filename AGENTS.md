...
- **The course-support Codex responder owns provider coverage incidents.** A lightweight production inspection runs every 2 minutes and starts full responder work only when actionable work exists. Up to two unrelated provider groups may remain owned concurrently, matching the global provider request limit and preserving Codex capacity for releases and other work; provider-family leases still serialize actual provider traffic.
