...
- Any Codex thread may create a linked Git worktree without requesting user approval when isolation is useful. Creating a new clone or independent repository folder still requires explicit user approval.
...
- If unrelated work makes branch creation unsafe in the current checkout, do not switch, stash, or absorb it. Create an isolated linked worktree from current `origin/main` without asking for approval, and give that worktree its own task branch.
...
- **The course-support Codex responder owns provider coverage incidents.** A lightweight production inspection runs on the gated 15-minute schedule and starts full responder work only when actionable work exists. The scheduled automation invokes the stable preflight with `--run --scheduled-cycle` so the one fast command records worker health and can stop immediately when both slots are occupied; it must not launch a second inspection or load the full responder contract for that busy tick. Up to two unrelated provider groups may remain owned concurrently, matching the global provider request limit and preserving Codex capacity for releases and other work; provider-family leases still serialize actual provider traffic.
...
The existing `tee-time-spot-course-support-responder` automation is the sole course-support responder. It runs on a gated 15-minute schedule and exclusively owns open `NEEDS_ADAPTER` / `FETCH_FAILED` incidents. Improve this automation in place; do not create, resume, or overlap a second course-support automation. Start every task with:
...
- The stable preflight must resolve `@playwright/test` from the selected approved checkout and verify that checkout's configured Chromium executable before inspection or claim. Missing browser runtime is a privacy-safe `setup_required` stop; do not launch a browser, inspect production work, or claim a batch first.
...
- The persisted per-entry claim action plan is authoritative. Follow its `primaryAction`, and use another command only when that action appears in the same entry's `allowedActions`; the batch-wide remediation directive explains the route but does not expand it. `IMPLEMENT_REUSABLE_SUPPORT` must claim and commit at least one runtime-bearing adapter/parser/registry path, with tests or notes alongside it as needed, and rejects `--current-runtime`; `ADVANCE_DISCOVERY` must advance a different unattempted safe stage; `VERIFY_TRANSIENT` is bounded; `COMPLETE_CLASSIFICATION` requires current proof; and `WAIT_FOR_MATERIAL_CHANGE` is not claimable work. Never substitute unchanged-runtime verification or a docs/tests-only release for a required reusable implementation.
- For an unfamiliar course, preserve retained source and booking evidence, verify exact identity, and search Google Places before browser work. Exact source-free state means the incident, course, and resolved provider families are all `SOURCE_MISSING`, both `Course.website` and `Course.detectedBookingUrl` are null, and no runnable provider capability exists. That state may assign `SEARCH_FOR_OFFICIAL_SOURCE` independently of the retained incident kind or failure class. The owned responder may issue one exact name/address/locality web-search context only when that action is allowed for the current source-free cycle, then persist either one direct safe public candidate or a no-unique-source result under the same batch/cycle fence. This private research context never belongs in the ordinal packet, automation notes, operator DTO, aggregate output, or final task report. A candidate remains untrusted until the guarded browser path validates it; the append-only source-search event must not directly update `Course`, and any later accepted projection uses the current ownership, incident-cycle, revision, and source-snapshot compare-and-set fences.
- Rendered and independent browser investigations may attempt at most 12 prioritized same-origin pages through depth two plus at most three exact course-scoped booking destinations, including same-origin booking destinations. Evaluate identity per page so sibling-course text cannot contaminate evidence. Every public request is signed-out and limited to `GET`, `HEAD`, or `OPTIONS`; abort every other method, never submit a form, and never sign in, solve a challenge, enter a queue or checkout, or book. Safe booking CTA controls may be inspected for a public destination but never activated as a transaction. Persist only sanitized page/CTA URL shapes, evidence categories, and value-free public request fingerprints whose query data is key-only; never persist cookies, request/response payloads, full HTML, screenshots, credentials, raw inline/widget configuration, search-result payloads, or checkout state.
- After claim, external browser MCP, DevTools, and ad-hoc browser commands are diagnostic only and cannot satisfy a responder stage. Persisted rendered discovery and independent confirmation must run through the exact owner-bound `verify --watch --closeout` path. An exact unchanged-runtime `ADVANCE_DISCOVERY` / `REPAIR_PROVIDER_ADAPTER` assignment at `BROWSER_ADAPTER_RETRY` belongs to that verifier; if the provider remains non-runnable, record the retry `NOT_APPLICABLE` and advance to `LOCAL_READER` instead of selecting provider inspection or repeating an orchestration-only assignment. Persist only recognized Playwright timeout or transient browser/network failures as stage attempts; programming, invariant, ownership, persistence, abort, and unknown failures must escape to the durable task/error lane.
- Only when an owned entry's persisted action plan includes `INSPECT_PROVIDER_CONTRACT`, use `npm run automation:course-support -- inspect-provider-contract --batch-ref <batch-ref> --ordinal 01`; it is a conditional diagnostic, not a required step for every implementation or discovery route. Do not compose ad-hoc shell, DevTools, or caller-supplied URL/bundle commands. Do not invoke it for a `SOURCE_MISSING` or `SOURCE_CONFLICT` sentinel with no trusted landing; follow the assigned action, and use `source-search-context` only when its exact gate explicitly permits it. The command accepts only the owned batch reference and ordinal (plus the ordinary owner-task proof), returns only actionable sanitized current-cycle XHR/fetch contracts bound to the exact resulting provider snapshot first, and otherwise may read one owner-derived family-consistent public booking landing (falling back to the official site) plus one same-origin non-executed script through DNS pinning and provider-request coordination. Document and static-asset traffic never suppresses that bounded fallback. The landing is parsed once, so only a real executable `script[src]` resolved through its safe effective base can supply the asset; account controls, actions, refresh metadata, or challenge state stop inspection. Static source is never executed and yields a clue only under the whole-program allowlist of inert strings and direct, decoded, statically bounded bare `fetch` statements; an unprovable URL, declaration, wrapper, Axios use, assignment, dynamic code, or other executable statement makes the bundle non-authorizing. Every authorization read atomically checks the lease against live database time, binds the ordinal to its one claimed remediation attempt, current playbook event count, provider snapshot, failure identity, and non-final monitoring state, and binds the stable technical identity of every other batch member; only after that whole-batch authority matches may same-route new evidence replace fallback work. Browser persistence separately carries the exact pre-observation provider fingerprint, and only a transaction-proven projection from that base may bind contracts to the resulting snapshot; unrelated drift remains unbound while coarse restriction evidence stays fail-closed. Its output is limited to bounded enums, generic path templates, query-key categories, digests, aggregates, and reason codes. It never returns raw origins, URLs, values, bodies, headers, cookies, tokens, or course identity; never enters account, challenge, queue, or checkout surfaces; never mutates course/evidence/incident/batch/playbook state; and never satisfies a playbook stage or the runtime-bearing implementation requirement. Provider-request coordination may temporarily write its lease row, but that infrastructure lease is not domain evidence.
- Treat `inspect-provider-contract` control outcomes literally. `recovery_required` is reserved for exact batch owner, lease token, allowed status, or database-time lease-expiry loss. `route_ineligible` returns the persisted `assignedAction`; follow it instead of forcing inspection. `authority_drift` means the claimed technical identity changed and requires a fresh owned packet before any action. `lease_headroom_insufficient` means the lease is still owned but too short for the bounded request budget; renew or heartbeat it and retry the diagnostic, not recovery.
- Scheduled responders use the one already-approved dispatch checkout on a clean `automation/course-support-*` branch whose `HEAD` equals current `origin/main`; persist the real owner task and base SHA, inspect the ordinal-only packet, then claim every planned path before editing it. Up to two unrelated classification/verification batches may overlap, but only one active batch may enter `IMPLEMENTING` or own planned code paths in the shared checkout. Heartbeat the 15-minute batch lease and persist the candidate release SHA immediately after commit.
- The responder uses the transaction-scoped `tee-time-spot:course-support-writer` lease only for short inspect, claim, recovery, and closeout transitions. Up to two durable classification/verification batches may own different provider-family/failure-fingerprint groups concurrently; the same group must never have two owners, and the shared checkout permits only one code-changing `IMPLEMENTING` owner.
- Every gated 15-minute responder launch must use the approved persistent dispatch checkout. Concurrent owners may perform only fenced database/browser verification unless one has exclusive `IMPLEMENTING` authority; no task may fast-forward or mutate the checkout while another batch is implementing. The hourly automation keeps its separate persistent checkout and must never share a mutable Git worktree with a responder.
- After a claim identifies its provider family, read `docs/course-support-provider-notes/README.md` and only that selected family's note when one exists. Do not scan every provider note. The Postgres incident, attempt ledger, monitoring events, and batch proof remain authoritative; Markdown is a privacy-safe engineering memory that prevents an agent from repeating a disproven approach. Create or update only the selected family note when the batch produces reusable implementation knowledge, proves a prior approach wrong, or changes the material conditions that permit a retry. Routine transient retries, unchanged verification, and per-course facts do not justify note churn.
...
- After the candidate release SHA has been persisted by the required pre-push heartbeat, use the owner-bound `automation:course-support -- verify-release --batch-ref <batch-ref> --release-sha <git-sha>` command from the clean claimed branch. It waits for the exact Git-created Ready deployment and both production aliases, persists deployment proof under the current lease, and then runs the verification watch and closeout. It does not replace the pre-push release-SHA fence. Do not split the later changed-release handoff into an unowned deployment wait plus a verifier. Unchanged-runtime routes keep their explicit current-runtime deployment proof flow.
...
- The one-time parked-course campaign is admitted through dry-run-first `automation:course-discovery-recheck -- --parked-cohort`. It stores the exact 112-member baseline, digest, capture time, and aggregate evidence categories in immutable `AutomationRun.audit`, admits at most five same-family/fingerprint courses through ordinary batch ownership and evidence paths, and leaves current active real demand ahead of requestless campaign work. Ordinary work wins by default, but after three consecutive completed noncritical batches whose supported schema explicitly proves they are noncampaign, the next eligible claim reserves one ready requestless campaign group. An active or recent campaign batch, or any missing, legacy, or malformed active/recent campaign summary, suppresses or resets that reservation. The Serializable claim transaction must re-read and fence the exact last-three history, active batches, and newly due active real demand before campaign admission. Lifecycle completion requires fresh campaign-tagged post-capture terminal proof for all 112 members plus a dynamically recomputed generic parked/waiting count of zero; it must never infer completion from a current label alone. The immutable cohort's automatic-within-24-hours rate is a diagnostic rather than a rollout gate. Operational acceptance requires at least 95% of future unfamiliar cycles to reach an explicitly automatic endpoint within 24 hours, the 30-day rolling human-review rate to be at most 5%, and repeat deployed implementations for the same provider-family/failure-fingerprint group to be zero. Unknown or incomplete metric provenance is not a pass. Report only aggregate counts, digest, categories, and metric statuses, never the membership or exact course identity context.
...
- Detached verification counts as execution only after the owned `PRE_EXECUTION` boundary sets `CourseSupportVerificationRequest.startedAt`; request claim, Workflow attachment, and dispatch attempts are not provider evidence. Preserve that boundary across every release in the batch: a later release must not forget an earlier started request, deployment, provider proof, or terminal proof, and every orchestration-only closeout must CAS-fence all release-scoped requests before writing. A cycle with no started request, deployment, provider proof, or playbook progress remains automatic, consumes no provider no-progress budget, and must not become per-course human review. Persist its orchestration-only evidence and retry on the fixed 15-minute responder cadence; retain the increasing orchestration attempt number only for observability, never to lengthen that delay. This infrastructure exception does not excuse a skipped schema-v1 `IMPLEMENT_REUSABLE_SUPPORT` action: unless authoritative terminal evidence or a material provider change supersedes the claim, stopped or direct closeout must fail before scheduling another retry until the runtime-bearing path, committed descendant release, and deployment proof are recorded. Fail the owning automation run and return a failing cron status when dispatch fails so one system-health signal replaces ambiguous course classifications.
- New schema-v1 closeouts persist exact `actionExecution`. Implementation counts as executed only with runtime-bearing planned and committed paths plus descendant release and deployment proof; runtime verification and classification require their exact current proof. Authoritative success, material change, or a terminal result may explicitly supersede an implementation without claiming it executed. Source-search and provider-contract actions remain execution `UNAVAILABLE` until their exact action markers exist. Owner deployment/verification failure checkpoints are resumable operational evidence only and never provider, course, or progress proof.
- Retry only `RATE_LIMIT`, `HTTP_5XX`, `TIMEOUT`, and `NETWORK` on the persisted 15-minute, 1-hour, 6-hour, then 24-hour ladder with jitter, for at most four unchanged attempts; honor bounded rate-limit `Retry-After`. Structural discovery/parser/adapter failures never use that generic ladder. When the same approach has no new material input, move it to `NEEDS_HUMAN`/`ENGINEERING_VERIFICATION_NEEDED` without queuing another search or Codex task. The six-hour visibility timer and unrelated deployments do not restart the playbook. Explicit recovery requires expired ownership and exact branch/HEAD/dirty-path provenance.
- After provider/playbook execution begins, each owned verification stage has a 30-minute endpoint. If another different safe playbook stage remains, deadline reconciliation preserves the cycle and evidence, refreshes one bounded stage deadline, and keeps the incident `AUTO_INVESTIGATING`; a timer alone must not fabricate ambiguous course-level human work. Exit automatic investigation only for restored monitoring, a durable factual/technical/source endpoint, a genuinely exhausted unchanged stage, or explicit operator evidence that requires judgment. A durably proven zero-execution infrastructure pause follows the separate system-health rule above and does not fabricate a course endpoint.
- If expired work cannot be adopted because clean checkout provenance moved, the documented recovery command may atomically close and requeue it only when no candidate release is published on current `origin/main`, no deployment or dispatch was recorded, and no terminal evidence or human decision exists. A persisted candidate SHA alone is not a published release. Never use this path for dirty or partially released work, and never delete or rewrite the abandoned local branch.
...
Exact source research may corroborate the retained official URL. The one-candidate budget still applies, and recording that candidate does not accept its identity or modify the course: fresh independent course/locality proof is required. When that owned stage learns runnable provider metadata, its current-release, snapshot-bound discovery may proceed to detached provider verification in the same batch. An unchanged exhausted provider remains subject to the implementation handoff; discovered metadata alone is never monitoring success.
...
Acquire the hourly-specific transaction-scoped `tee-time-spot:hourly-improvement-writer` lease before candidate selection, path claims, or closeout. It serializes only hourly transitions; responder activity is not `blocked_concurrent`. The single unfinished hourly owner `AutomationRun` is the durable guard for the rest of the run. Keep `outcome_recorded=false` during preparation, editing, and verification; set it true only in the same closeout write that records `completedAt` and the terminal outcome. Terminal exceptions must close the owned run with a redacted error and concrete blocker.
...
- `docs/course-support-provider-notes/` is the privacy-safe reusable provider-learning index. Its files never replace live Postgres state or current provider observation. Read the index plus only the selected family note, and update only that family when a responder batch produces reusable worked/failed knowledge.
