...
- If expired unreleased work cannot be adopted because clean checkout provenance moved, the documented recovery command may atomically close and requeue it only when no release, deployment, dispatch, terminal evidence, or human decision exists. Never use this path for dirty or partially released work, and never delete or rewrite the abandoned local branch.
