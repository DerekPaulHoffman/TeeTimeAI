...
- An engineering-only incident may remain `AUTO_INVESTIGATING` for at most 30 minutes. By that deadline it must be monitored, durably classified as a verified terminal limitation, or moved to `ENGINEERING_VERIFICATION_NEEDED` with completed attempts and one exact next action.
