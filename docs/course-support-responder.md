...
- The required automation configuration for `tee-time-spot-course-support-responder` is 24 hours a day, every 2 minutes, with `C:\dev\TeeTimeAI-course-support-dispatch` as its actual working directory. Verify that configuration in the automation control plane; this document does not prove that the live automation has already been updated.
...
- Before production inspection, run the current responder preflight with `npm run automation:course-support:preflight -- --run`. It reuses an approved prepared checkout when available and returns a structured setup requirement otherwise. Do not create a replacement checkout merely because setup is required.
...
5. In the automation control plane, verify the primary responder is the sole active 24/7 two-minute schedule, uses the exact approved `C:\dev\TeeTimeAI-course-support-dispatch` checkout, invokes the current preflight and scheduled `inspect --scheduled-cycle`, and may dispatch up to five overlapping task executions that each own exactly one batch. Shared-checkout coordination must still permit only one `IMPLEMENTING` owner at a time; the other tasks may inspect, claim, wait, or verify without adopting that implementation scope. Verify that the duplicate overnight responder is paused. Do not infer configuration success from this document.
