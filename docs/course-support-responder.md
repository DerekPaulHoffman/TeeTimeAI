...
For discovery entries that authorize both current-runtime verification and provider-contract inspection, the primary action is `VERIFY_CURRENT_RUNTIME`. Run the existing owner-bound `verify --current-runtime --watch --closeout` lane so provider observations advance the playbook and reach durable closeout. Contract inspection remains an optional diagnostic; an empty diagnostic result is not a completed stage or a conclusive course limitation. Implementation-only, exact source-search, ownership, and public-access requirements still apply unchanged.
