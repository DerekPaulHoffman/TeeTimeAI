...
- Recognized but non-runnable families are `EZLINKS`, `GOLFNOW`, and `TENFORE`. Recognition is not proof of monitoring support.
...
- `Course.monitoringMode=LOCAL_READER_ONLY` is a durable routing decision for an exact allowlisted rendered-page reader. It bypasses ordinary HTTP/browser discovery and server adapters in both golfer checks and detached responder verification. If the configured booking URL no longer maps to an allowlisted reader, fail as tooling/configuration work instead of trying another provider path.
...
When an expired batch cannot be adopted because its checkout provenance moved, `recover` refreshes `origin/main` and may safely requeue the batch instead of blocking the lane indefinitely, but only when the checkout is clean, no candidate release is published on current `origin/main`, no deployment or recheck dispatch was recorded, and every batch entry is still pending or stale. A persisted candidate SHA is not by itself a published release. The transition does not adopt, reset, publish, or delete the local branch. It atomically closes the abandoned batch as retryable, releases each incident, records a short future retry, and closes the unfinished automation run. Any dirty checkout, published or deployed release, dispatch activity, terminal evidence, human decision, or concurrent state change still fails closed for owner attention.
