...
- The required automation configuration for `tee-time-spot-course-support-responder` is 24 hours a day, every 2 minutes, with `C:\dev\TeeTimeAI-course-support-clean` as its actual working directory. Verify that configuration in the automation control plane; this document does not prove that the live automation has already been updated.
