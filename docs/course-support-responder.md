...
- Scheduled automation runs `npm run automation:course-support:preflight -- --run --scheduled-cycle`; manual read-only diagnosis uses `npm run automation:course-support:preflight -- --run` without the scheduled flag. Preflight fetches current `origin/main`, preserves dirty or diverged checkouts for their owner, safely fast-forwards a clean approved checkout, refreshes dependencies only when the lockfile changed, and selects the primary or standby. It returns a structured setup requirement when neither approved checkout is safe. Do not create an unapproved replacement checkout merely because setup is required.
...
- Scheduled inspection emits one aggregate-only `handoff` with strict `RECOVER` then `RESUME` then `CLAIM` then `STOP` precedence. An ordinary dispatch group wins before campaign work. When the immutable parked campaign is `RUNNING` with ready members, a writer slot is available, and the ordinary diagnostic group list is empty, inspection emits `{ action: "CLAIM", source: "PARKED_CAMPAIGN", maxCourses: 5, selection: "ATOMIC_SERVER_SIDE" }`; the existing responder runs the normal production-wrapped claim and lets the claim transaction choose the exact member. The handoff never contains a provider, fingerprint, course, incident, or batch identifier, and preflight never claims work itself.
...
Failure to start detached provider execution is infrastructure evidence, not course evidence. `CourseSupportVerificationRequest.startedAt` is set only at the owned `PRE_EXECUTION` boundary; claiming or attaching a Workflow does not set it. Release advancement retains a bounded, monotonic summary of prior deployment, provider, and terminal execution; closeout CAS-fences every request from every release, so a later release cannot erase or race an earlier execution boundary. A current-cycle closeout with no started request, deployment, provider proof, or playbook progress remains `AUTO_INVESTIGATING`, does not consume the provider no-progress budget, and does not create a human-review endpoint. It records append-only orchestration provenance, retries after an exponential delay beginning at 15 minutes and capped at six hours, marks the owning automation run failed for system-level visibility, and makes a failed dispatcher cron return a failing HTTP status. Once any provider/deployment/playbook execution begins, the ordinary bounded evidence and endpoint rules apply. Recurring course observations preserve an intentional infrastructure pause only while the latest completed batch proves this exact zero-execution state.

## Unfamiliar-Course Source And Browser Recovery

The responder treats an unfamiliar course the way a careful signed-out visitor would, while keeping every step bounded and reproducible. It preserves retained official/booking evidence, corroborates the exact course identity, uses Place Details and one strict exact-name/address/locality Google Places search, and only then advances to rendered work. When the current owned source-missing cycle still has no usable source, `source-search-context` exposes one exact query and one attempt reference only to the current batch owner. That private identity context is never included in the ordinal packet, `AutomationRun` notes, operator DTOs, aggregate output, or the final task report.
...
# five same-family/fingerprint members when no ordinary due work is waiting.
