...
When an expired batch cannot be adopted because its checkout provenance moved, `recover` may safely requeue it instead of blocking the lane indefinitely, but only when the checkout is clean, no release or deployment was recorded, no recheck dispatch began, and every batch entry is still pending or stale. The transition does not adopt, reset, publish, or delete the local branch. It atomically closes the abandoned batch as retryable, releases each incident, records a short future retry, and closes the unfinished automation run. Any dirty checkout, persisted release/deployment, dispatch activity, terminal evidence, human decision, or concurrent state change still fails closed for owner attention.
