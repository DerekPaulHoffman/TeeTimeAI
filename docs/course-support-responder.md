...
A genuinely source-free course stays on that ordered discovery lane after rendered discovery as well. The owned verifier must establish whether the adapter retry and local reader apply, then finish independent confirmation. Do not assign an unidentified provider implementation merely because those later stages are pending. This does not reset an exhausted playbook, authorize another source search, or establish restored monitoring; missing-source evidence remains unresolved until the native proof supports an outcome.

If a source-free course reaches independent confirmation without a usable source, close out to a fresh `SEARCH_FOR_OFFICIAL_SOURCE` assignment before opening a browser. The source-search command still owns its exact query, budget, immutable candidate and snapshot fences. A no-unique result appends only the remaining independent research result; it must preserve the completed earlier stages. Browser confirmation requires the candidate recorded under that assignment.
...
- Local inspection passes its verified checkout commit to campaign admission selection only when the checkout is clean, on an eligible named branch, and `HEAD` matches `origin/main`. The production environment wrapper may omit deployment identity; its `local` fallback must not hide work that the exact checkout can safely claim. Outside that verified checkout, manual diagnostics retain the ordinary no-override behavior. This selection-only value is never persisted as campaign evidence, does not change environment or deployed-runtime identity, and cannot prove provider execution or course acceptance. Claim and deployment verification retain their independent fences.
...
When a complete candidate-history read proves that one course exceeds the per-course current-cycle event limit, that course remains fenced while independently proven candidates may proceed. Claim output and the batch summary expose `candidateHistoryBlockedCount`, and the task stays visible for that history review. No events are deleted or rewritten. An aggregate read overflow, invalid history, or a queue with no independently proven candidates still fails closed.
...
At `BROWSER_ADAPTER_RETRY`, a current-cycle, exact-provider-snapshot, family-consistent successful read-only browser XHR/fetch observation with an actionable availability-shaped contract may promote the assigned action to `IMPLEMENT_REUSABLE_SUPPORT`. The marker retains only contract digest and count and is rederived atomically with routing from authoritative browser evidence. Restrictions, stale or mismatched evidence, and unrelated page-shell traffic remain fail-closed and cannot promote. This marker is routing evidence only: it does not count as `inspect-provider-contract` execution, provider execution, or restored monitoring.

When all eight ordered stages end `UNRESOLVED_EXHAUSTED` but a trusted safe public landing still exists, settled-watch and endpoint closeout create one explicit `EXHAUSTED_DISCOVERY_IMPLEMENTATION_HANDOFF` instead of applying the generic provider retry ladder or detached-provider floor. Its primary action is `IMPLEMENT_REUSABLE_SUPPORT`; the exact persisted action plan may also allow the bounded `INSPECT_PROVIDER_CONTRACT` diagnostic. Eligible current actionable contract evidence is carried into the handoff when available. If it is absent, that one diagnostic may recover a contract from the already-authorized landing under its existing request, lease, and authority limits. This handoff is due after one minute only so the next normal responder launch can claim the different implementation action; it is not another discovery retry and does not prove course progress. Once claimed, the owner must either produce the full runtime-bearing implementation, release, deployment, and provider proof, accept a fresh authoritative or material-change winner, or use the existing exhaustion-gated human escalation. Without a material change, the same exact implementation handoff can never be assigned twice. A missing or inconclusive contract must never release the course into another scheduled retry.

If an exact schema-v1 independent-confirmation owner persists that final discovery result and stops before closeout, expired-owner deadline recovery preserves the same one-shot implementation handoff. It requires fresh exact-runtime browser-stage markers, the unchanged claimed source and incident identity, a valid current-cycle claim prefix, complete bounded claim/closeout history with no prior unchanged implementation selection, and transactional history, request, search, and ownership fences. The recovery receipt consumes only the completed discovery action and anchors the next selection to the current source; it never claims implementation execution or restored monitoring. Missing or conflicting modern proof stays fenced. This does not admit historical unowned human-review records, reset cycles, replay unchanged implementation, or bypass source-free, factual, or technical endpoints.

A completed discovery watch may refine its failure classification without changing the provider. Exact current-cycle claim/closeout and deployed-runtime stage evidence distinguish that refinement from a new material input; claimed and observed fingerprints remain intact. Every relevant current-cycle receipt must be readable before the selector or deadline reconciler can prove that implementation is unused. A later proven zero-execution verification cleanup preserves that source receipt. Deadline reconciliation retains the resulting implementation handoff and its existing retry time, without repeatedly extending its deadline or counting provider execution. Recovery from human-review state is limited to the exact automatically emitted exhausted-discovery deadline event, matching current identity and evidence, with no operator decision. Unknown, manual, malformed, already-consumed, source-free, and conclusive outcomes are not reopened by this exception.
...
A validated missing-source or retained-source research handoff does not wait for the old detached continuation to execute the newly assigned action. Closeout still requires its exact failure evidence to be consumed, preserves active-request and success fences, and retires the old request in the same transaction that releases the course for a fresh source-search claim.

The existing course-name/address/locality context and result commands enforce the live claim, lease, operator decision, ledger, source snapshot and runtime fences. The single query uses ordinary identifying terms, without mandatory quoted names, full mailing-address phrases, or a quoted generic phrase such as “official golf course”; official pages may format those facts differently. Search results remain untrusted candidates: exact course identity and locality must still be established by owned browser verification. One result is keyed to the incident cycle and rejection evidence across batches. If context returns `searchBudget=0`, no further web search is allowed: continue the stored candidate through the owned browser verifier. Changed query/identity, assignment or source evidence must not silently reuse an old result.

The existing recovery cron recognizes a material query capability change only when a current-cycle owned `NO_UNIQUE` result matches the obsolete quoted-identity query digest, the unchanged course identity/source and its exhausted closeout. It admits at most five affected courses per pass into a fresh ordinary discovery cycle, once per course for this query implementation. Prior query results, playbook events and course identities remain intact. Active owners, operator decisions, working provider metadata, newer success and conclusive access/identity evidence remain authoritative. A deployment alone does not reopen an investigation; older six-hour exhausted schedules do not substitute for this proof.

Both source-research commands identify the executing local CLI from its clean Git checkout and require the claimed branch and exact batch release (or base when no release is recorded). They do not depend on Vercel runtime environment variables, which the local production wrapper may omit. Context validates this before returning a query; result recording revalidates it instead of trusting the earlier context. This is local tooling provenance, not deployed provider execution. The native transactional ownership, source-snapshot and immutable-result fences remain authoritative.

When discovery learns a runnable provider, fresh exact-runtime provider verification bound to that resulting snapshot can establish `MONITORING_RESTORED` through normal batch closeout. The metadata change from the original claim is expected and does not invalidate that newer proof. Closeout still rechecks the live request, owner, provider snapshot and search state; a newer monitoring failure or changed provider snapshot supersedes an older success.
...
A single provider failure stays quiet while the two-minute retry runs. A confirmed outage sends one customer-safe notice saying the provider's public service is not responding, the alert remains active, and Tee Time Spot will keep retrying. Confirmation may come from two failures on the course or a matching provider-family failure on another course. A fresh successful public check owes a recovery notice to the current authorized recipients even when no earlier outage email reached them. A matching-times email showing that course can satisfy the notice. Only the courses actually shown as monitored count; pending-source copy and invisible full-search snapshots do not. The generation-scoped outbox owns delivery retries and deduplication.

### Customer recovery after provider repair

New batches carry `customerRecoveryVersion: 1`. Native closeout records a separate `summary.customerRecovery` case while holding the affected search locks. Resolving the provider incident does not close this case: each affected active customer alert needs two distinct successful provider observations from different check runs, a healthy scheduled continuation, and an accepted email showing each repaired course for every current recipient in the current alert generation. Signed local-reader observations must satisfy the same provider-execution evidence contract as hosted checks. A successful first observation, a queued email, a dry run, a hidden status snapshot, or an older generation cannot satisfy recovery.

Every native `inspect`, including the scheduled preflight, refreshes up to 32 open cases on completed batches using bounded persisted evidence. Cases remain open across scheduled runs without reopening healthy course incidents. Recipient addresses, search IDs, and provider payloads are never copied into the recovery summary. Owner edits reset the evidence floor once; removed recipients and ended demand stop imposing delivery requirements. `NO_ACTIVE_DEMAND` means customer delivery was not applicable, not that an email was sent. Detached verification continues to prove provider readiness only.

Always read `customerRecovery` in the inspection and closeout output, including when the provider queue says `no_due_work`. A pending case is an unfinished customer outcome. Diagnose its provider, installed-reader, scheduler, or email-outbox failure and keep the case visible; preserve native ownership, action-plan, release, and technical-access fences. Do not downgrade a healthy course or consume provider discovery attempts to represent a shared-system delivery failure. If the bounded work cannot repair that component, retain an explicit engineering handoff and the open case. Never send a separate manual email to make the acceptance check pass.
...
The first post-deploy `verify` may create a detached request and report only aggregate `detachedVerification.pendingCount` plus `detachedVerification.rerunNeeded`. Scheduled owners use `verify --watch --closeout`: one owner-bound command renews the lease, persists every eligible browser stage, waits for the one-minute deployed request recovery, re-verifies, requires two consecutive clean browser-stage scans, and performs derived proof-gated closeout. The second clean scan catches a detached result that advances the ledger into independent confirmation after the prior pass's browser scan. The command envelope keeps `browserStages` as the final convergence pass and reports summed action counts from every completed watch pass in `browserStageTotals`; a final clean-pass zero therefore cannot hide browser work performed earlier in the same watch. These totals count stage targets per pass and may include the same target on more than one pass, while `persistedCount` remains the durable-write count. The watch fails closed on timeout or ownership/release/stage change and never marks an unexhausted playbook human. Closeout also fails closed while the exact-release request is queued or checking, while a zero-execution ordered stage or concluded adapter/reader recovery still has a scheduled continuation, when a success has not yet been consumed, or when current provider-executed retry/cooldown evidence has not been copied into the batch proof. This prevents an immediate closeout from cancelling the check, orphaning a bounded continuation result, or discarding a provider `Retry-After`.

Known closeout freshness guards request bounded re-verification under the same owner instead of immediately releasing the batch into an early retry. The watch permits at most three such refreshes within its original deadline and requires two new clean passes before trying closeout again. Only typed incident-verification freshness and detached-request readiness/consumption guards receive this authority; ownership, release, invalid proof, and unknown errors remain failures. The watch reports `evidenceRefreshRetryCount` and an allowlisted `lastEvidenceRefreshReason`, and stopped closeout retains those fields without raw errors. Refreshing evidence is orchestration, not provider execution or a new implementation attempt.

Successful ownership cleanup does not make failed verification successful. `verify` and `verify-release` exit nonzero when the watch stops with an error or cleanup records `command_failed`, retaining the complete durable cleanup result. The release wrapper reports `release_verification_failed` in that case rather than `release_verification_completed`. An ordinary proof-derived endpoint retry remains distinct from a command failure and does not become restored monitoring merely because the command exits zero.
...
For the direct-started-reader path, a legacy member with six completed stages and a still-`STARTED` `LOCAL_READER` stage may use one `STARTED_LOCAL_READER_CONTINUATION` in its existing cycle. This is explicit permission to finish current work, not repaired historical lineage or a claimed provider capability change. It requires the latest native legacy claim to select that exact stage, unchanged current source/failure snapshots, a settled owner and a stale `FETCH_FAILED` request with explicit zero execution, complete bounded history, and a later exact parking endpoint. Native deadline endpoints from `RECOVERY_CRON`, `SEARCH_WORKFLOW`, or `COURSE_SUPPORT_RESPONDER` must explicitly record `playbookExhausted: false`; legacy omission remains allowed only for `COURSE_SUPPORT_RESPONDER`. A missing flag is never rewritten. In both cases the valid current ledger independently proves incomplete work, the endpoint must follow the settled batch and request, and later incident evidence prevents using it. Operator endpoints, unknown producers, explicit exhaustion and null flags remain excluded. A currently fresh compatible reader must support the required parser, and every active search, live verification request, unexpired reader job, stronger final evidence, or ownership/evidence drift fences admission. Reader eligibility follows the verifier's allowlisted local-reader route, not the availability of a separate server adapter; `SERVER_ONLY`, `CONTACT_ONLY`, conflicting provider evidence and non-actionable access remain excluded. The Serializable write rechecks the immutable membership, reader availability using database time, source, ledger, batches, requests and monitoring events. It preserves the cycle, every prior event and attempt counter, and appends a one-shot receipt keyed to incident/cycle/stage rather than release. Exhausted stages are excluded. Native selection, claim and exact deployed-runtime verification still apply; admission alone is neither execution nor a resolved course.

A second, explicitly labeled `LEGACY_UNANCHORED_READER_EVIDENCE_RENEWAL` proof may use the same one-shot permission when the immutable legacy claim selected `BROWSER_ADAPTER_RETRY` and its exact owned request advanced through `NOT_APPLICABLE`/`NO_RUNNABLE_ADAPTER` to `LOCAL_READER` `STARTED`. The settled stale request must retain its null outcome and null evidence; prior provider execution remains `UNKNOWN`, never zero. One coherent, expired, schema-valid historical `NO_AVAILABILITY` reader result must bind to that request's date/player tuple and current reader capability, pass native public-read-only URL, identity, parser and timing checks, and lack the server anchor. It is historical data, not monitoring success. Selection and the Serializable write bind and recheck its full-row fingerprint. The actual owned verification request must use a later course-local date and a distinct canonical job key; the queue preserves both the historical result and any already-existing new-key row. This does not reopen exhausted work, repair old evidence, create another recovery budget or bypass any existing source, ownership, active-work, safety or exact-runtime gate.

The parked-course campaign is a bounded use of the existing responder, not another scheduler or sweep automation. A dry run must first observe exactly 112 eligible parked members and return the membership digest. `--apply` accepts only that exact count and current digest, then stores the sorted membership, capture time, digest, and aggregate evidence categories as immutable `AutomationRun.audit`. The responder selects ordinary due work before campaign work by default and keeps current active real demand ahead of requestless campaign work. To prevent an indefinitely replenished historical queue from starving the immutable cohort, three consecutive completed noncritical batches whose supported summaries explicitly prove they are noncampaign reserve the next eligible claim for one ready requestless campaign group. An active or recent campaign batch, or any missing, legacy, or malformed active/recent campaign summary, suppresses or resets that reservation. Within the requestless campaign lane, a member whose bounded incomplete playbook has safely advanced is selected ahead of an untouched member so the campaign produces end-to-end outcomes instead of breadth-first stage churn; entries at the same stage retain attempt and age fairness, and active customer demand remains authoritative. The Serializable claim transaction re-reads and fences the exact history window, active ownership, and newly due active real demand before it admits a reserved member. Every campaign batch still admits no more than five same-provider-family/failure-fingerprint members through the ordinary batch path.

A first admitted cycle may refine its failure class and fingerprint while keeping the same provider family and provider snapshot. That is evidence progression, not provider execution. A one-shot `FAILURE_REFINEMENT_INCOMPLETE_PLAYBOOK_RECOVERY` may resume only the next unfinished stage when the complete monitoring-event history since campaign capture proves the single original admission, the exact bounded-playbook marker, a valid incomplete append-only ledger, a non-overlapping current-cycle batch history, a started `FETCH_FAILED` request for the refined snapshot, and an exact same-cycle summary in that batch or a later batch. The Serializable write re-derives and locks the immutable campaign audit, course snapshot, every batch and request row, latest probe/discovery evidence, and complete campaign-window event set before appending its recovery marker. It preserves cycle, ledger, attempt counts, timestamps, and operator evidence. A prior marker, changed provider snapshot, reversed or overlapping request/summary order, exhausted ledger, missing exact summary, or truncated history fails closed. This recovery marker never claims provider execution, restored monitoring, or a terminal course outcome.
...
Rollout completion therefore has two independent gates. The original-cohort gate requires all 112 immutable members to have fresh campaign-proven working monitoring or a conclusive factual/technical limitation; hiding, parking, reassignment, or ambiguous retry does not count. The future-system gate requires the rolling automatic-endpoint, human-review, and implementation-reuse metrics below to pass from actual provider execution and terminal evidence. Passing one gate never substitutes for the other.
...
# Read-only diagnostic for the immutable campaign's acceptance gaps. It uses
# the existing strict terminal classifier inside a RepeatableRead, READ ONLY
# transaction. Only --machine is accepted and explicit DATABASE_URL is required.
# Output is aggregate-only; UNKNOWN never becomes zero. Partition PASS means
# accounting consistency, not monitoring acceptance. Incomplete-stage counts
# do not grant automatic admission, ownership, or authority to requeue work.
npx tsx scripts/automation/inspect-course-support-acceptance-ledger.ts --machine
