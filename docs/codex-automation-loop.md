...
- Every successful closeout must record a non-empty result for `operations_errors`, `browser_location`, `feedback_discord_behavior`, `missing_course_search`, `current_practice_research`, and `product_quality`. A result may be healthy, empty with sample counts, unavailable with an exact blocker, or actionable, but it may not be omitted. The same access gap in three successful runs becomes a durable coverage blocker rather than another harmless note.
