...
- Claim one homogeneous provider-family/failure-fingerprint batch from a clean `automation/course-support-*` branch at current `origin/main`. Read the ordinal packet, claim every path before editing it, heartbeat the 15-minute lease, fence the candidate release SHA immediately after commit, and use explicit provenance-checked recovery after expiry. If unfamiliar-provider implementation needs network-contract clues, use only the owner-bound ordinal `inspect-provider-contract` command; it accepts no caller target or payload, prefers current sanitized XHR/fetch evidence carrying the exact post-projection provider-snapshot fingerprint, and can inspect at most one server-derived DNS-pinned non-executed script without persisting evidence or satisfying a playbook stage. Static document/script traffic falls through, and any claimed-attempt, playbook-count, provider-snapshot, failure-identity, monitoring-state, owner, or lease drift stops before provider I/O.
