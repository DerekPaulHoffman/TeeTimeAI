...
- Every 2 minutes: inspect the course-support queue and claim at most one due bounded batch. Up to five independent scheduled tasks may own unrelated groups concurrently; each task retains one batch through its bounded verification watch. Active real demand remains higher priority, but engineering-only work may also claim when initial customer endpoints are not waiting. Routine no-work or busy results stop before branch creation, dependency installation, or provider research.
- Checkout and lane isolation: the responder uses the checkout selected from its approved primary/standby pool with `tee-time-spot:course-support-writer`; up to five verification/classification tasks may overlap, but only one active batch may own code paths or enter `IMPLEMENTING`. The bounded improvement loop uses `C:\dev\TeeTimeAI-automation` with `tee-time-spot:hourly-improvement-writer`. Each durable owner blocks duplicates only inside its own lane.
