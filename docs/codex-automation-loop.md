...
5. Load current evidence from open `CourseSupportIncident` rows, `WebsiteEvent`, `WebsiteFeedback`, recent learning signals, `CourseAutomationDiscovery`, current probes, smoke evidence, deployment notes, and recent Vercel logs. For candidate preparation, choose the newest probe per active course/search across all outcomes before filtering to actionable failures; a later success or no-match supersedes an earlier failure.
