...
- Email delivery: Resend sends only real tee-time match alerts to the search owner and their chosen additional recipients. Setup, morning status, monitoring transition, operator, and automation-worker health email are disabled; those states remain visible in the dashboard and `/operator`.
...
- Course support: the first unresolved `NEEDS_ADAPTER` or `FETCH_FAILED` result opens a persistent internal `CourseSupportIncident` only after official-site discovery has run. The existing `tee-time-spot-course-support-responder` is the sole course-support automation; improve its gated 15-minute flow in place and do not create, resume, or overlap another responder. It groups due incidents by provider family and failure fingerprint, with 5 courses by default and an absolute limit of 20. It restores reusable public read-only monitoring or records a conclusive technical-access/contact/identity disposition. Ordinary one-check `TEST`/`AUTOMATION` searches do not create incidents; explicitly opted-in `syntheticMultiCycle` coverage creates `engineeringOnly` incidents that remain actionable after the source search ends but never send customer or operator support email. Real demand promotes the same incident to customer-demand priority. See [Course-Support Responder](./course-support-responder.md).
- Responder checkout safety: scheduled preflight selects only from the approved primary/standby pool. A dirty or diverged checkout is quarantined in place for its owner; a clean behind checkout may be fast-forwarded and have lockfile-declared dependencies refreshed. When neither checkout is safe, the responder fails closed and the overdue worker is elevated at the top of `/operator` while unresolved investigations remain.
- Official-site discovery retains known sources, checks Place Details, and performs one strict exact-name/address/locality Google Places search before rendered work. Exact source-free state requires the incident, course, and resolved provider families to be `SOURCE_MISSING`, both persisted source roots to be null, and no runnable provider capability; that state may route to source search independently of the retained incident kind or failure class. When the owned entry's persisted action plan assigns `SEARCH_FOR_OFFICIAL_SOURCE`, only its owner may request the private exact identity context, perform the single focused web search, and append either one direct safe public candidate or a no-unique-source result under the current ownership/cycle/snapshot fence. The raw query and result payload never enter packets, automation notes, operator DTOs, aggregate output, or task reports, and a candidate cannot update `Course` until guarded browser validation. Rendered and independent investigations attempt at most 12 prioritized same-origin pages through depth two plus at most three exact course-scoped booking destinations. They use signed-out `GET`, `HEAD`, and `OPTIONS` only; every other method is aborted, forms are never submitted, and sign-in, challenge, queue, checkout, and booking flows are never entered. Identity is evaluated per page, and persisted network fingerprints retain method/status/origin/path/query-key names without values or payloads. Explicit official private-club plus member-and-guest access evidence becomes an official-site-only blocked classification; explicit course-level statements that tee times are not required and play is first-come, first-served become `WALK_IN` classifications. Multi-use TeeSnap pages are enriched from their public browser-facing configuration and must match the requested course by name while excluding colocated range or simulator inventory, and TeeItUp links are canonicalized to the provider booking root instead of retaining store or gift-certificate paths.
- Every 15 minutes: run the stable production preflight and inspect the course-support queue. Empty or healthy-busy cycles stop before loading the full responder contract. An actionable cycle claims at most one due bounded batch, and up to two independent scheduled tasks may own unrelated groups concurrently; each task retains one batch through its bounded verification watch. Active real demand remains higher priority, but engineering-only work may also claim when initial customer endpoints are not waiting. Missed ticks are not replayed as a burst.
- Parked-course campaign: ordinary due batches are selected before the immutable 112-member parked cohort by default, and current active real demand always remains ahead of requestless campaign work. After three consecutive completed noncritical batches whose supported summaries explicitly prove they are noncampaign, the next eligible claim reserves one ready requestless campaign group. An active or recent campaign batch, or any missing, legacy, or malformed active/recent campaign summary, suppresses or resets that bounded reservation. The Serializable claim re-reads and fences the exact last-three history, active batches, and newly due active real demand before admitting the campaign member. The cohort is admitted only through the dry-run count/digest fence and reuses the same ownership, evidence, provider-learning, verification, and closeout paths. Durable campaign completion requires fresh campaign-tagged terminal proof for every captured member and a current generic parked/waiting count of zero. The baseline automatic-within-24-hours rate is an SLA diagnostic, not a rollout gate. Rollout acceptance additionally requires at least 95% of future unfamiliar cycles to resolve automatically within 24 hours, no more than 5% human-review endpoints across the rolling 30-day window, and zero repeat deployed implementations for one provider-family/failure-fingerprint group; incomplete provenance is `UNKNOWN`, not a pass. Expose only aggregate campaign counts, digest, evidence categories, and metric statuses.
- Checkout and lane isolation: the responder uses the checkout selected from its approved primary/standby pool with `tee-time-spot:course-support-writer`; up to two verification/classification tasks may overlap, matching the provider I/O ceiling, but only one active batch may own code paths or enter `IMPLEMENTING`. The bounded improvement loop uses `C:\dev\TeeTimeAI-automation` with `tee-time-spot:hourly-improvement-writer`. Each durable owner blocks duplicates only inside its own lane.
...
- Claim one homogeneous provider-family/failure-fingerprint batch from a clean `automation/course-support-*` branch at current `origin/main`. The versioned action plan persisted for each entry is authoritative: follow its `primaryAction`, and use only alternatives listed in its `allowedActions`; do not derive a different command from the retained incident kind, failure class, or batch-wide directive. Read the ordinal packet, claim every path before editing it, heartbeat the 15-minute lease, fence the candidate release SHA immediately after commit, and use explicit provenance-checked recovery after expiry. Use the owner-bound ordinal `inspect-provider-contract` command only when that entry's action plan allows it. The diagnostic accepts no caller target or payload, prefers current sanitized XHR/fetch evidence carrying the exact post-projection provider-snapshot fingerprint, and can inspect at most one server-derived DNS-pinned non-executed script without persisting evidence or satisfying a playbook stage. Its `recovery_required` result is reserved for exact owner/token/allowed-status or database-time lease-expiry loss; `route_ineligible` returns the assigned action to follow; `authority_drift` requires a refreshed packet; and `lease_headroom_insufficient` requires lease renewal or heartbeat rather than recovery. Static document/script traffic falls through, and any claimed-attempt, playbook-count, provider-snapshot, failure-identity, monitoring-state, owner, or lease drift stops before provider I/O.
...
- Close unresolved provider-executed transient failures with the persisted 15-minute, 1-hour, 6-hour, then 24-hour retry ladder, with jitter and bounded `Retry-After` handling. A proven orchestration-only closeout instead retries at a fixed 15 minutes; its increasing attempt number is observability only and never lengthens the delay. Missing execution for a schema-v1 `IMPLEMENT_REUSABLE_SUPPORT` assignment is not orchestration-only: absent authoritative terminal evidence or material provider change, closeout fails before it can schedule another retry until the claimed runtime-bearing implementation, descendant release, and deployment proof exist. Never archive a retryable result without a future `nextAttemptAt`.
