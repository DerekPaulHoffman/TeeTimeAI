...
- Hourly: run one complete autonomous improvement cycle whenever no responder batch actively owns or must recover the repository writer lane. A due course-support backlog without an owner does not block unrelated product work and remains excluded from the hourly portfolio. Inspect every non-course-support evidence track, rank structured candidates, select one portfolio category, implement one coherent batch, verify it, commit it, push it, deploy live-impacting work, verify production, and record what was learned.
...
1. Inspect the responder queue and acquire the shared repository-writer transition lease. Stop `blocked_concurrent` before candidate selection when a responder batch owns or must recover the writer lane. Record an unowned due backlog as informational, never select it as hourly work, and continue with unrelated evidence tracks. After hourly preparation, treat the unfinished owner `AutomationRun` as the durable rest-of-run guard.
