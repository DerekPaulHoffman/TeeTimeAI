...
5. Load current evidence from open `CourseSupportIncident` rows, `WebsiteEvent`, `WebsiteFeedback`, recent learning signals, `CourseAutomationDiscovery`, current probes, smoke evidence, deployment notes, and recent Vercel logs. Use the inspector's 24-hour event counts by traffic class to evaluate page views, course-discovery outcomes, and alert creation without treating automation traffic as public funnel evidence. For candidate preparation, choose the newest probe per active course/search across all outcomes before filtering to actionable failures; a later success or no-match supersedes an earlier failure.
