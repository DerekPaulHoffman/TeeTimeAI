...
- Official-site discovery may follow up to two clearly scored booking-link handoffs after the course homepage. Explicit official private-club plus member-and-guest access evidence becomes an official-site-only blocked classification, multi-use TeeSnap pages are enriched from their public browser-facing configuration and must match the requested course by name while excluding colocated range or simulator inventory, and TeeItUp links are canonicalized to the provider booking root instead of retaining store or gift-certificate paths.
