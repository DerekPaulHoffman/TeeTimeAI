...
- Checkout isolation: the responder uses `C:\dev\TeeTimeAi-CourseSupportResponder`; the bounded improvement loop uses `C:\dev\TeeTimeAI-automation`. The separate approved checkouts prevent pre-claim branch races, while the shared Postgres writer lease prevents overlapping release ownership.
- Every six hours: run one bounded autonomous improvement cycle whenever no responder batch actively owns or must recover the repository writer lane. Inspect every non-course-support evidence track, rank structured candidates, and ship one coherent verified batch only when a candidate clears the evidence and safety threshold. Otherwise record `no_action_healthy`; never manufacture churn to satisfy the schedule.
