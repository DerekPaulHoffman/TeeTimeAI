...
- Every 10 minutes: inspect the course-support queue and claim at most one due bounded batch. Active real demand remains higher priority, but engineering-only work may also claim on every run so the responder continuously drains the queue. Routine no-work or busy results stop before branch creation, dependency installation, or provider research.
