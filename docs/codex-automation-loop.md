...
- Treat `AutomationRun.notes`, `WebsiteEvent`, `WebsiteFeedback`, recent browser discoveries, probe history, and deployment notes as a living learning ledger. Saved `TeeSearch` demand carries the same aggregate `PUBLIC`, `UNCLASSIFIED`, `AUTOMATION`, or `TEST` provenance as funnel events; synthetic demand remains inspectable but must not outrank customer incidents, pending alerts, or active-search counts. Each loop should record what went right, what went wrong, what assumption changed, and the next concrete action.
