...
Tee Time Spot uses Postgres as the durable source of user demand, per-search Vercel Workflows for operational checks, a dedicated 10-minute Codex responder for course-support engineering, and a separate hourly Codex loop for broad product improvement.
...
- Every 10 minutes: inspect the course-support queue. Exit and auto-archive a durably recorded routine result before branch creation, dependency installation, or provider research. When work is due, claim and heartbeat one provider-family/fingerprint batch, verify against fresh deployed-runtime evidence, and close it durably.
- Checkout isolation: the responder uses `C:\dev\TeeTimeAI-cohort-remediation`; the hourly loop uses `C:\dev\TeeTimeAI-automation`. The separate approved worktrees prevent pre-claim branch races, while the shared Postgres writer lease prevents overlapping release ownership.
- Hourly: run one complete autonomous improvement cycle only when no responder batch is active and no course-support work is due. Inspect every non-course-support evidence track, rank structured candidates, select one portfolio category, implement one coherent batch, verify it, commit it, push it, deploy live-impacting work, verify production, and record what was learned.
...
- Acquire the shared transaction-scoped `tee-time-spot:repository-writer` lease before hourly preparation, path claims, and closeout. It serializes transitions with the responder; the single unfinished owner `AutomationRun` is the durable rest-of-run guard. Due responder work, an active responder batch, or an unfinished hourly run inside its active window is `blocked_concurrent`.
...
1. Inspect the responder queue and acquire the shared repository-writer transition lease. Stop `blocked_concurrent` before candidate selection when course-support work is due or owned; after hourly preparation, treat the unfinished owner `AutomationRun` as the durable rest-of-run guard.
