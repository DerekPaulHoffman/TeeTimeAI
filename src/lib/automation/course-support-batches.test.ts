...
describe("course-support closeout playbook decision basis", () => {
  it("reports a canonical joint next-stage and attempt histogram", () => {
    const result = buildCourseSupportCloseoutPlaybookDecisionBasis({
      incidents: [
        {
          cycle: 1,
          attemptLedger: browserAdapterRetryFailedRetryableAttemptLedger(1, 2),
        },
        { cycle: 1, attemptLedger: { version: 1, events: [] } },
        { cycle: 1, attemptLedger: typedAdapterReadyAttemptLedger() },
        { cycle: 1, attemptLedger: typedAdapterReadyAttemptLedger() },
      ],
    });

    expect(result).toEqual({
      playbookAssessmentAvailableIncidentCount: 4,
      playbookAssessmentInvalidIncidentCount: 0,
      playbookExhaustedCount: 0,
      incompletePlaybookCount: 4,
    ...
    expect(
      result.incompletePlaybookNextStageAttemptHistogram?.reduce(
        (total, entry) => total + entry.incidentCount,
        0,
      ),
    ).toBe(result.incompletePlaybookCount);
  });

  it("keeps a proven zero Browser Adapter retry and complete playbook distinct", () => {
    const result = buildCourseSupportCloseoutPlaybookDecisionBasis({
      incidents: [
        { cycle: 1, attemptLedger: browserAdapterRetryReadyAttemptLedger() },
        { cycle: 1, attemptLedger: exhaustedAttemptLedger() },
      ],
    });

    expect(result).toMatchObject({
      playbookAssessmentAvailableIncidentCount: 2,
      playbookAssessmentInvalidIncidentCount: 0,
      playbookExhaustedCount: 1,
      incompletePlaybookCount: 1,
      zeroAttemptBrowserAdapterRetryCount: 1,
      incompletePlaybookNextStageAttemptHistogram: [
        {
          nextStage: "BROWSER_ADAPTER_RETRY",
          attemptCount: 0,
  ...
  it("fails every incomplete-derived aggregate closed on invalid current-cycle evidence", () => {
    const result = buildCourseSupportCloseoutPlaybookDecisionBasis({
      incidents: [
        { cycle: 1, attemptLedger: typedAdapterReadyAttemptLedger() },
        { cycle: 1, attemptLedger: { version: 2, events: [] } },
      ],
    });
    ...
    expect(result).toMatchObject({
      ...
      incompletePlaybookNextStageAttemptHistogram: null,
    });
  });
});
...
beforeEach(() => {
  vi.clearAllMocks();
  prismaMocks.transaction.mockReset().mockImplementation(
    async (
...
describe("course-support batch selection", () => {
  ...
  it("fails closed when candidate demand exceeds the bounded preference read", () => {
    const preferences = Array.from({ length: 1_025 }, (_, index) => ({
      teeSearch: {
        id: `search-${index}`,
        date: new Date("2026-07-20T00:00:00.000Z"),
      },
    ...
    expect(() => deriveCourseSupportCurrentDemand(preferences)).toThrow(
      "candidate demand exceeds the bounded read limit",
    );
  });
...
describe("course-support claim demand fencing", () => {
  ...
  it("routes current provider evidence after a watchdog invalidates the stale deferred identity", async () => {
    ...
    await expect(
      claimCourseSupportBatch({
        ownerThreadId: "owner-thread-current-after-invalidation",
        branch: "automation/course-support-self-healing",
        baseSha,
        now: fixture.dueAt,
      }),
  ...
  it("lets only the source-CAS winner claim a deferred confirmation", async () => {
    const fixture = deferredFailureCandidateFixture();
    prismaMocks.queryRaw.mockResolvedValue([{ now: fixture.dueAt }]);
    prismaMocks.supportIncidentFindMany
      .mockResolvedValueOnce([fixture.incident])
    ...
    await expect(
      claimCourseSupportBatch({
        ownerThreadId: "owner-thread-deferred-race",
        branch: "automation/course-support-self-healing",
        baseSha,
        now: fixture.dueAt,
      }),
  ...
  it("lets malformed newest history shadow an older valid deferred source", async () => {
    ...
    await expect(
      claimCourseSupportBatch({
        ownerThreadId: "owner-thread-deferred-shadow",
        branch: "automation/course-support-self-healing",
        baseSha,
        now: fixture.dueAt,
      }),
  ...
  it("fences the incident and monitoring revisions again inside deferred claim", async () => {
    const fixture = deferredFailureCandidateFixture();
...
describe("fresh runtime verification", () => {
  ...
  it("rejects an exact-runtime reused provider result whose source time predates dispatch", () => {
    expect(
      classifyFreshBatchEvidence({
        batchCreatedAt: now,
  ...
  it("rejects a re-stamped local-reader result whose source predates the current incident", () => {
    const result = classifyFreshBatchEvidence({
      batchCreatedAt: now,
      incidentLastSeenAt: new Date("2026-07-15T20:10:30.000Z"),
      deployedAt: new Date("2026-07-15T20:05:00.000Z"),
      releaseSha: "a".repeat(40),
      recheckDispatchStartedAt: new Date("2026-07-15T20:10:00.000Z"),
      newestProbe: {
        id: "re-stamped-local-reader-after-incident",
        outcome: "NO_MATCH",
        observedAt: new Date("2026-07-15T20:11:00.000Z"),
        freshSearchCheckedAt: new Date("2026-07-15T20:11:00.000Z"),
        runtimeVersion: "a".repeat(40),
        providerExecution: true,
        providerExecutionObservedAt: new Date(
          "2026-07-15T20:10:15.000Z",
        ),
      },
      course: runnableCourse,
    });
...
describe("search-specific remediation proof", () => {
  ...
  it("uses the newest contradictory probe so an older success cannot hide a later failure", () => {
    const earlierAt = new Date("2026-07-15T20:10:30.000Z");
    const laterAt = new Date("2026-07-15T20:10:45.000Z");
    const proof = collectFreshRemediatedCourseProof({
      searches: [
        searchEvidence("FETCH_FAILED", {
          probes: [
            {
              id: "older-success",
              courseId: "course-1",
  ...
  it("accepts a newer success after an older provider failure", () => {
    const earlierAt = new Date("2026-07-15T20:10:30.000Z");
    const laterAt = new Date("2026-07-15T20:10:45.000Z");
    const proof = collectFreshRemediatedCourseProof({
      searches: [
  ...
  it("orders a delayed-written older local-reader success behind a source-newer failure", () => {
    const olderProviderAt = new Date("2026-07-15T20:10:30.000Z");
    const newerProviderAt = new Date("2026-07-15T20:10:45.000Z");
    const delayedWriteAt = new Date("2026-07-15T20:10:55.000Z");
    const proof = collectFreshRemediatedCourseProof({
      searches: [
        searchEvidence("FETCH_FAILED", {
          probes: [
            {
              id: "delayed-older-local-reader-success",
              courseId: "course-1",
              outcome: "NO_MATCH",
              observedAt: delayedWriteAt,
              runtimeVersion: releaseSha,
              providerExecution: true,
              providerExecutionObservedAt: olderProviderAt,
            },
            {
              id: "source-newer-failure",
              courseId: "course-1",
              outcome: "FETCH_FAILED",
              observedAt: newerProviderAt,
              runtimeVersion: releaseSha,
              providerExecution: true,
            },
          ],
  ...
  it("accepts a source-newer success ahead of a delayed-written older local-reader failure", () => {
    const olderProviderAt = new Date("2026-07-15T20:10:30.000Z");
    const newerProviderAt = new Date("2026-07-15T20:10:45.000Z");
    const delayedWriteAt = new Date("2026-07-15T20:10:55.000Z");
    const proof = collectFreshRemediatedCourseProof({
      searches: [
        searchEvidence("NO_MATCH", {
          probes: [
            {
              id: "delayed-older-local-reader-failure",
              courseId: "course-1",
              outcome: "FETCH_FAILED",
              observedAt: delayedWriteAt,
              runtimeVersion: releaseSha,
  ...
  it.each([
    ...
    "fails closed on contradictory equal-source probes with ids %s and %s",
    (successId, failureId) => {
      const tiedSourceAt = new Date("2026-07-15T20:10:45.000Z");
      const proof = collectFreshRemediatedCourseProof({
        searches: [
          searchEvidence("FETCH_FAILED", {
            probes: [
              {
                ...
                courseId: "course-1",
                outcome: "NO_MATCH",
                observedAt: new Date("2026-07-15T20:10:55.000Z"),
                runtimeVersion: releaseSha,
                providerExecution: true,
                providerExecutionObservedAt: tiedSourceAt,
              },
              {
                id: failureId,
                courseId: "course-1",
                outcome: "FETCH_FAILED",
                observedAt: new Date("2026-07-15T20:10:50.000Z"),
                runtimeVersion: releaseSha,
                providerExecution: true,
                providerExecutionObservedAt: tiedSourceAt,
              },
  ...
  it("fails closed when a third equal-source probe contradicts two newer-row successes", () => {
    const tiedSourceAt = new Date("2026-07-15T20:10:45.000Z");
    const proof = collectFreshRemediatedCourseProof({
      searches: [
        searchEvidence("NO_MATCH", {
          probes: [
            ...
            {
              id: "second-row-success",
              courseId: "course-1",
              outcome: "NO_MATCH",
              observedAt: new Date("2026-07-15T20:10:54.000Z"),
              runtimeVersion: releaseSha,
              providerExecution: true,
              providerExecutionObservedAt: tiedSourceAt,
            },
            {
              id: "third-row-contradictory-failure",
              courseId: "course-1",
              outcome: "FETCH_FAILED",
              observedAt: new Date("2026-07-15T20:10:53.000Z"),
              runtimeVersion: releaseSha,
              providerExecution: true,
              providerExecutionObservedAt: tiedSourceAt,
            },
    ...
    expect(proof.freshProviderProofByCourse.has("course-1")).toBe(false);
    expect(proof.healthyCourseSearchPairCountByCourse.get("course-1")).toBe(
      undefined,
    );
  });

  it("uses the stable id tie-break only when equal-source probes have the same meaning", () => {
    const tiedAt = new Date("2026-07-15T20:10:45.000Z");
    const proof = collectFreshRemediatedCourseProof({
      searches: [
        searchEvidence("NO_MATCH", {
          probes: [
  ...
  it("rejects a fresh probe row whose local-reader source observation predates dispatch", () => {
    const proof = collectFreshRemediatedCourseProof({
      searches: [
        searchEvidence("NO_MATCH", {
          probes: [
            {
              id: "re-stamped-local-reader",
              courseId: "course-1",
              outcome: "NO_MATCH",
              observedAt: checkedAt,
              runtimeVersion: releaseSha,
    ...
    expect(proof.freshProviderProofByCourse.has("course-1")).toBe(false);
    expect(proof.freshProviderAttemptByCourse.has("course-1")).toBe(false);
  });
...
describe("remediated scheduler health", () => {
  ...
  it("rejects a start reservation until the workflow run is attached", () => {
    for (const checkStatus of ["WAITING", "CHECKING"]) {
      expect(
        isRemediatedSearchSchedulerHealthy(
          {
            status: "ACTIVE",
            ...
            nextCheckAt: new Date("2026-07-15T20:31:00.000Z"),
    ...
    incidents: incidentStates.map((status, index) => {
      ...
      return {
        ...
        incident: {
          ...
          lastSeenAt: new Date("2026-07-15T19:20:00.000Z"),
          cycle: 2,
          activeBatchId: "batch-1",
          updatedAt,
        },
...
function expiredPreExecutionRecoveryFixture(input: {
  status: "QUEUED" | "CHECKING" | "STALE";
  updatedAt: Date;
  ...
  const startedAt =
    input.status === "QUEUED"
      ? null
      : new Date(input.updatedAt.getTime() - 30_000);
  const request = {
    id: "request-pre-execution",
    batchIncidentId: entry.id,
    releaseSha: expiredBatch.baseSha,
    runtimeVersion:
      input.status === "QUEUED" ? null : expiredBatch.baseSha,
    status: input.status,
    revision: 3,
    leaseToken: input.status === "CHECKING" ? "request-lease" : null,
    leaseExpiresAt:
      input.status === "CHECKING" ? (input.leaseExpiresAt ?? null) : null,
    attemptCount: 1,
    startedAt,
    outcome: null,
    failureClass: null,
    evidence: null,
    lastError:
      input.status === "STALE" ? "pre_execution_stopped" : null,
    providerSnapshotFingerprint: "b".repeat(64),
    nextAttemptAt: null,
    completedAt: input.status === "STALE" ? input.updatedAt : null,
    createdAt: new Date(input.updatedAt.getTime() - 120_000),
    updatedAt: input.updatedAt,
  };
  Object.assign(entry, { verificationRequests: [request] });
  return { expiredBatch, request };
}

function configureExpiredSchemaV1ImplementationRecovery(
  expiredBatch: ReturnType<typeof expiredPreExecutionRecoveryFixture>["expiredBatch"]
) {
  const entry = expiredBatch.incidents[0];
  const approach = {
    workMode: "IMPLEMENT_REUSABLE_SUPPORT" as const,
    strategyAction: "REPAIR_PROVIDER_ADAPTER" as const,
    playbookStage: "OFFICIAL_IDENTITY" as const
  };
  expiredBatch.summary.remediation = {
    ...
    attempts: [
      {
        ...
        providerSnapshotFingerprint: "b".repeat(64),
        failureFingerprint: expiredBatch.failureFingerprint,
        runtimeVersion: expiredBatch.baseSha,
        activeRealSearchCount: 0,
        playbookEventCountAtClaim: 0,
        reason: "IMPLEMENTATION_REQUIRED",
        retryBudget: null,
...
describe("course-support recovery", () => {
  ...
  it.each([
    ...
    {
      label: "expired CHECKING",
      status: "CHECKING" as const,
      updatedAt: new Date(now.getTime() - 30_000),
      leaseExpiresAt: new Date(now.getTime() - 1),
    },
  ])(
    "does not recover an expired batch while detached verification is $label",
    async ({ status, updatedAt, leaseExpiresAt }) => {
  ...
  it("safely requeues a stable PRE_EXECUTION-only expired request", async () => {
    const courseRef = createHash("sha256")
      .update("course-1")
      .digest("hex")
      .slice(0, 24);
    ...
    const requestUpdatedAt = new Date(now.getTime() - 30_000);
    ...
    Object.assign(entry, { verificationRequests: [preExecutionRequest] });
    prismaMocks.batchFindUnique.mockResolvedValue(expiredBatch);
    prismaMocks.verificationRequestFindMany.mockResolvedValue([
      preExecutionRequest,
    ]);
    ...
    expect(prismaMocks.verificationRequestUpdateMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: expect.objectContaining({
          id: preExecutionRequest.id,
          revision: preExecutionRequest.revision,
          startedAt: preExecutionRequest.startedAt,
          updatedAt: requestUpdatedAt,
        }),
        data: {
          revision: { increment: 0 },
    ...
    expect(persistedAttempt).toMatchObject({
      ...
      orchestrationRetry: expect.objectContaining({ attemptNumber: 1 }),
    });
  });

  it("persists exact action execution when safely requeueing a schema-v1 verification", async () => {
    const requestUpdatedAt = new Date(now.getTime() - 30_000);
    ...
    await expect(
      ...
      safelyRequeued: true,
      durableCloseoutRecorded: true,
      nextAttemptAt: new Date(now.getTime() + 15 * 60_000).toISOString()
    });
    ...
    expect(persistedAttempt?.orchestrationRetry).toMatchObject({
      attemptNumber: 1,
      retryAt: new Date(now.getTime() + 15 * 60_000).toISOString()
    });
  });

  it("durably parks expired schema-v1 implementation when a fresh success is only scheduling evidence", async () => {
    const { expiredBatch, request } = expiredPreExecutionRecoveryFixture({
      status: "STALE",
      updatedAt: new Date(now.getTime() - 30_000)
    });
    const entry = configureExpiredSchemaV1ImplementationRecovery(expiredBatch);
    entry.incident.lastSeenAt = new Date(now.getTime() - 2 * 60_000);
    entry.course.probes = [
      {
        id: "fresh-scheduling-only-success-probe",
        outcome: "NO_MATCH",
        observedAt: new Date(now.getTime() - 10_000),
        runtimeVersion: expiredBatch.baseSha,
        rawSummary: {
          bookingWindow: { releaseDate: "2026-07-29", exactTime: true },
        },
      },
    ];
    expiredBatch.ownerAutomationRunId = "run-stopped-implementation";
    prismaMocks.batchFindUnique.mockResolvedValue(expiredBatch);
    prismaMocks.verificationRequestFindMany.mockResolvedValue([request]);
    prismaMocks.verificationRequestUpdateMany.mockResolvedValue({ count: 1 });
    prismaMocks.batchUpdateMany.mockResolvedValue({ count: 1 });
        ...
        data: expect.objectContaining({
          ...
          summary: expect.objectContaining({
            closeout: expect.objectContaining({
              ...
              reason: "expired_implementation_stopped_without_proof"
            })
          })
        })
      })
  ...
  it("requeues expired schema-v1 implementation when a source-newer success outranks a delayed-written older local-reader failure", async () => {
    const { expiredBatch, request } = expiredPreExecutionRecoveryFixture({
      status: "STALE",
      updatedAt: new Date(now.getTime() - 30_000)
    });
    ...
    entry.course.probes = [
      {
        id: "delayed-older-recovery-failure-probe",
        outcome: "FETCH_FAILED",
        observedAt: new Date(now.getTime() - 5_000),
        runtimeVersion: expiredBatch.baseSha,
        rawSummary: {
          providerExecution: "LOCAL_BROWSER_READER",
          providerObservedAt: new Date(now.getTime() - 20_000).toISOString(),
        },
      },
      {
        id: "fresh-recovery-success-probe",
        outcome: "NO_MATCH",
        observedAt: new Date(now.getTime() - 10_000),
        runtimeVersion: expiredBatch.baseSha,
        rawSummary: {
          providerExecution: "RUNNABLE_PROVIDER_CHECK",
          providerObservedAt: new Date(now.getTime() - 10_000).toISOString(),
        }
      }
    ];
    entry.incident.lastSeenAt = new Date(now.getTime() - 2 * 60_000);
    prismaMocks.batchFindUnique.mockResolvedValue(expiredBatch);
    prismaMocks.courseProbeFindMany.mockResolvedValue(entry.course.probes);
    prismaMocks.verificationRequestFindMany.mockResolvedValue([request]);
    prismaMocks.verificationRequestUpdateMany.mockResolvedValue({ count: 1 });
    prismaMocks.batchUpdateMany.mockResolvedValue({ count: 1 });
    prismaMocks.incidentUpdateMany.mockResolvedValue({ count: 1 });
    ...
    expect(prismaMocks.monitoringStatusUpdateMany).not.toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ state: "ENGINEERING_VERIFICATION_NEEDED" })
      })
    );
  });

  it("does not let a delayed-written older local-reader success supersede a source-newer failure during recovery", async () => {
    const { expiredBatch, request } = expiredPreExecutionRecoveryFixture({
      status: "STALE",
      updatedAt: new Date(now.getTime() - 30_000),
    });
    ...
    entry.course.probes = [
      {
        id: "delayed-older-recovery-success-probe",
        outcome: "NO_MATCH",
        observedAt: new Date(now.getTime() - 5_000),
        runtimeVersion: expiredBatch.baseSha,
        rawSummary: {
      ...
      {
        id: "source-newer-recovery-failure-probe",
        outcome: "FETCH_FAILED",
        observedAt: new Date(now.getTime() - 10_000),
        runtimeVersion: expiredBatch.baseSha,
        rawSummary: {
    ...
    expect(prismaMocks.incidentUpdateMany).not.toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ result: "RETRY_SCHEDULED" }),
      }),
    );
    ...
    expect(prismaMocks.supportIncidentUpdateMany).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          activeBatchId: null,
          nextAttemptAt: new Date(now.getTime() + 60_000),
          lastSeenAt: entry.incident.lastSeenAt,
        })
      })
    );
    expect(prismaMocks.monitoringStatusUpdateMany).not.toHaveBeenCalledWith(
  ...
  it("aborts schema-v1 implementation recovery when authoritative success appears during the transaction reread", async () => {
    const { expiredBatch, request } = expiredPreExecutionRecoveryFixture({
      status: "STALE",
      updatedAt: new Date(now.getTime() - 30_000)
    });
    const entry = configureExpiredSchemaV1ImplementationRecovery(expiredBatch);
    entry.incident.lastSeenAt = new Date(now.getTime() - 2 * 60_000);
    const concurrentSuccessProbe = {
      id: "concurrent-recovery-success-probe",
    ...
    prismaMocks.courseProbeFindMany
      .mockResolvedValueOnce([])
      .mockResolvedValueOnce([concurrentSuccessProbe]);
    prismaMocks.batchFindUnique.mockResolvedValue(expiredBatch);
    prismaMocks.verificationRequestFindMany.mockResolvedValue([request]);
    prismaMocks.verificationRequestUpdateMany.mockResolvedValue({ count: 1 });
    ...
    await expect(
      ...
      "implementation supersession evidence changed during safe requeue"
    );
  ...
  it("aborts schema-v1 implementation recovery when material provider supersession changes during the transaction reread", async () => {
    const { expiredBatch, request } = expiredPreExecutionRecoveryFixture({
      status: "STALE",
      updatedAt: new Date(now.getTime() - 30_000)
    });
    ...
    const currentCourse = {
      ...entry.course,
      providerFamilyKey: "CONCURRENT_PROVIDER_FAMILY"
    };
    verificationMocks.buildCourseSupportProviderSnapshotFingerprint.mockImplementation(
      (course: unknown) =>
        course === currentCourse ? "c".repeat(64) : "b".repeat(64)
    );
    prismaMocks.courseFindUnique.mockResolvedValue(currentCourse);
    prismaMocks.batchFindUnique.mockResolvedValue(expiredBatch);
    prismaMocks.verificationRequestFindMany.mockResolvedValue([request]);
    prismaMocks.verificationRequestUpdateMany.mockResolvedValue({ count: 1 });
  ...
  it("aborts stopped implementation recovery when terminal request evidence changes", async () => {
    const initialUpdatedAt = new Date(now.getTime() - 60_000);
    const { expiredBatch, request } = expiredPreExecutionRecoveryFixture({
      status: "STALE",
      updatedAt: initialUpdatedAt
    });
    configureExpiredSchemaV1ImplementationRecovery(expiredBatch);
    const freshRequest = {
      ...request,
      revision: request.revision + 1,
      lastError: "new_terminal_evidence",
      updatedAt: new Date(now.getTime() - 10_000)
    };
    prismaMocks.batchFindUnique.mockResolvedValue(expiredBatch);
    prismaMocks.verificationRequestFindMany.mockResolvedValue([freshRequest]);
  ...
  it("aborts stopped implementation recovery when monitoring CAS loses", async () => {
    const { expiredBatch, request } = expiredPreExecutionRecoveryFixture({
      status: "STALE",
      updatedAt: new Date(now.getTime() - 30_000)
    });
    ...
    ).rejects.toThrow("Course monitoring changed while stopping unproven implementation work");

    expect(prismaMocks.automationRunUpdateMany).not.toHaveBeenCalled();
  });

  it("aborts stopped implementation recovery when the owner run CAS loses", async () => {
    const { expiredBatch, request } = expiredPreExecutionRecoveryFixture({
      status: "STALE",
      updatedAt: new Date(now.getTime() - 30_000)
    });
    configureExpiredSchemaV1ImplementationRecovery(expiredBatch);
    expiredBatch.ownerAutomationRunId = "run-stopped-implementation-raced";
    prismaMocks.batchFindUnique.mockResolvedValue(expiredBatch);
    prismaMocks.verificationRequestFindMany.mockResolvedValue([request]);
    prismaMocks.verificationRequestUpdateMany.mockResolvedValue({ count: 1 });
    prismaMocks.batchUpdateMany.mockResolvedValue({ count: 1 });
    prismaMocks.incidentUpdateMany.mockResolvedValue({ count: 1 });
    prismaMocks.supportIncidentUpdateMany.mockResolvedValue({ count: 1 });
    prismaMocks.automationRunUpdateMany.mockResolvedValue({ count: 0 });
    ...
    ).rejects.toThrow("Expired responder automation run changed during safe requeue");
  });

  it("aborts safe requeue when a terminal request becomes CHECKING in the serializable reread", async () => {
    const initialUpdatedAt = new Date(now.getTime() - 60_000);
    const { expiredBatch, request } = expiredPreExecutionRecoveryFixture({
      status: "STALE",
      updatedAt: initialUpdatedAt,
    ...
    ).rejects.toThrow("still active during safe responder requeue");
...
describe("course-support inspection ownership", () => {
  ...
  it("reports live demand separately from historical real provenance", async () => {
    ...
    const dueInspectionQuery =
      prismaMocks.supportIncidentFindMany.mock.calls[0]?.[0];
    expect(dueInspectionQuery?.take).toBe(257);
    expect(
      dueInspectionQuery?.select?.course?.select?.preferences?.take,
    ).toBe(1025);
  });
...
describe("course-support release heartbeat persistence", () => {
  ...
  it("records a bounded owner failure checkpoint without closing the batch", async () => {
    prismaMocks.batchFindFirst.mockResolvedValue({
      ...ownedBatch(),
      status: "VERIFYING",
      summary: {
...
describe("detached verification atomic batch fences", () => {
  ...
  function activePreExecutionRequest(status: "QUEUED" | "CHECKING") {
    const updatedAt = new Date(now.getTime() - 30_000);
    return detachedRequestState(status, {
      id: `request-${status.toLowerCase()}`,
      revision: 2,
      leaseToken: status === "CHECKING" ? "request-lease" : null,
      leaseExpiresAt:
        status === "CHECKING" ? new Date(now.getTime() + 60_000) : null,
      attemptCount: status === "QUEUED" ? 0 : 1,
      startedAt:
        status === "QUEUED"
          ? null
          : new Date(updatedAt.getTime() - 30_000),
      outcome: null,
      failureClass: null,
      evidence: null,
      lastError: null,
      ...
      nextAttemptAt: null,
      completedAt: null,
      createdAt: new Date(updatedAt.getTime() - 120_000),
      updatedAt,
    });
  }
  ...
  it("does not persist verification after ownership cancellation reaches an in-flight transaction", async () => {
    const controller = new AbortController();
    const ownershipLost = new Error("Responder ownership renewal failed.");
    ...
    prismaMocks.transaction.mockImplementationOnce(
      async (
        ...
        return worker(monitoringTransactionClient);
      },
    );

    const verification = verifyCourseSupportBatch({
      batchId: "batch-1",
      leaseToken: "lease-1",
      ownerThreadId: "owner-thread",
      releaseSha,
    ...
    await expect(verification).rejects.toBe(ownershipLost);
    expect(prismaMocks.batchUpdateMany).not.toHaveBeenCalled();
    expect(prismaMocks.incidentUpdateMany).not.toHaveBeenCalled();
  ...
  it.each([
    ...
    },
    {
      ...
      requestedOutcome: "blocked_env" as const,
      failureDomain: "ENV" as const,
    },
  ])(
    "does not persist $verificationWatchMode closeout after ownership cancellation reaches an in-flight transaction",
    async (scenario) => {
      const controller = new AbortController();
      const ownershipLost = new Error("Responder ownership renewal failed.");
      let releaseTransaction = () => undefined;
      ...
      const closeout = closeoutCourseSupportBatch({
        batchId: "batch-1",
        leaseToken: "lease-1",
        ownerThreadId: "owner-thread",
        verificationWatchMode: scenario.verificationWatchMode,
        ...(scenario.requestedOutcome
          ? {
              requestedOutcome: scenario.requestedOutcome,
              failureDomain: scenario.failureDomain,
            }
          : {}),
        signal: controller.signal,
        now,
      ...
      await expect(closeout).rejects.toBe(ownershipLost);
      expect(prismaMocks.batchUpdateMany).not.toHaveBeenCalled();
      expect(prismaMocks.incidentUpdateMany).not.toHaveBeenCalled();
      expect(prismaMocks.supportIncidentUpdateMany).not.toHaveBeenCalled();
      expect(prismaMocks.monitoringStatusUpdateMany).not.toHaveBeenCalled();
  ...
  function batchSearchDispatch(
    ...
    providerExecutionMarker = "RUNNABLE_PROVIDER_CHECK",
        ...
        probes: [
          {
            ...
            evidenceUrl: null,
            rawSummary: {
              providerExecution: providerExecutionMarker,
  ...
  function productionFailureCooldownBatch(input: {
    ...
    const proof = {
      ...
      providerRetryNotBeforeAt: providerRetryNotBeforeAt.toISOString(),
      runtimeVersion: releaseSha,
      providerExecution: input.providerExecution ?? true,
      providerSnapshotFingerprint: providerFingerprint,
    };
    const requestEvidence = {
      ...proofEvidence(),
          ...
          attempts: [
            {
              ...
              playbookEventCountAtClaim:
                input.playbookEventCountAtClaim ?? 4,
              approach: {
                workMode: "VERIFY_TRANSIENT",
                strategyAction: "RUN_TYPED_ADAPTER",
                playbookStage: "BROWSER_ADAPTER_RETRY",
    ...
    Object.assign(batch.incidents[0].incident, {
      ...
      failureFingerprint: incidentFailureFingerprint,
      attemptCount: 2,
      attemptLedger: input.attemptLedger ?? attemptLedger,
      confirmedAt: new Date("2026-07-15T18:30:00.000Z"),
      firstSeenAt: new Date("2026-07-15T18:00:00.000Z"),
      updatedAt: observedAt,
    ...
    const request = detachedRequestState("RETRYABLE_FAILED", {
      ...
      revision: 2,
      attemptCount: 1,
      createdAt: new Date(observedAt.getTime() - 45_000),
  ...
  function deferredFailureCarrierBatch(input: {
    observedFailureClass: "MISSING_SOURCE" | "SCHEMA" | "HTTP_5XX";
    providerExecution: boolean;
    started: boolean;
    sourceRuntimeVersion?: string;
  }) {
    const sourceRuntimeVersion = input.sourceRuntimeVersion ?? "c".repeat(40);
    ...
    const fixture = productionFailureCooldownBatch({
      ...
      observedFailureClass: input.observedFailureClass,
      closeoutAt,
      batchCreatedAt: new Date("2026-07-15T20:35:00.000Z"),
      providerExecution: input.providerExecution,
      attemptLedger: exhaustedAttemptLedger(
        17,
        new Date("2026-07-15T20:34:00.000Z"),
      ),
      playbookEventCountAtClaim: 8,
    });
    const sourceObservedFailureFingerprint = buildProviderFailureFingerprint({
      providerFamilyKey: "booking.example",
      failureClass: "MISSING_SOURCE",
      operation: "AVAILABILITY",
    });
    const source = createDeferredFailureHandoffSignal({
      ...
      observedFailureFingerprint: sourceObservedFailureFingerprint,
      claimedProviderSnapshotFingerprint: providerFingerprint,
      observedProviderSnapshotFingerprint: providerFingerprint,
      runtimeVersion: sourceRuntimeVersion,
      cooldownExpiresAt: "2026-07-15T20:30:00.000Z",
      providerNotBeforeAt: "2026-07-15T20:29:00.000Z",
    ...
    fixture.batch.incidents[0].incident.batchIncidents = [
      {
        id: "source-entry",
        cycle: fixture.cycle,
        verificationRequests: [],
    ...
    if (!input.started) {
      fixture.batch.incidents[0].result = "PENDING";
      fixture.batch.incidents[0].proofSnapshot = null;
      fixture.batch.incidents[0].verifiedAt = null;
      fixture.request = detachedRequestState("STALE", {
        id: "request-deferred-unstarted",
        revision: 1,
        attemptCount: 0,
        runtimeVersion: null,
        startedAt: null,
        ...
        nextAttemptAt: null,
        completedAt: null,
        createdAt: new Date("2026-07-15T20:31:00.000Z"),
        updatedAt: new Date("2026-07-15T20:31:00.000Z"),
      });
    }
    return { ...fixture, admission, closeoutAt, source };
  }
  ...
  it("keeps unchanged-runtime provider failure execution out of orchestration retries", async () => {
    ...
    closingBatch.incidents[0].incident.failureFingerprint =
      decisionFailureFingerprint;
    closingBatch.incidents[0].incident.activeRealSearchCount = 0;
    Object.assign(
      closingBatch.summary.remediation.attempts[0],
      {
        failureFingerprint: decisionFailureFingerprint,
        runtimeVersion: releaseSha,
        activeRealSearchCount: 0,
    ...
    const closeoutResult = await closeoutCourseSupportBatch({
      batchId: "batch-1",
      leaseToken: "lease-1",
      ownerThreadId: "owner-thread",
      requestedOutcome: "blocked_env",
    ...
    expect(closeoutResult.decisionBasis).toMatchObject({
      verificationRequestStartedIncidentCount: 0,
      providerExecutionObservedIncidentCount: 1,
      providerExecutionAttemptRecordedIncidentCount: 1,
    });
  ...
  it("restores an active search from exact-runtime local-reader execution", async () => {
    ...
    prismaMocks.batchSearchFindMany.mockResolvedValue([
      batchSearchDispatch(
        searchRef,
        {
          id: "local-reader-provider-proof",
          outcome: "NO_MATCH",
          observedAt,
          runtimeVersion: releaseSha,
        },
    ...
    expect(prismaMocks.incidentUpdateMany).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          result: "RESTORED",
          postProbeId: "local-reader-provider-proof",
          proofSnapshot: expect.objectContaining({
            providerExecution: true,
            runtimeVersion: releaseSha,
          }),
  ...
  it("rejects a re-stamped local-reader result observed before dispatch", async () => {
    const searchRef = "e".repeat(64);
    const verifyingBatch = verificationBatch();
    Object.assign(verifyingBatch, {
      baseSha: releaseSha,
    ...
    prismaMocks.batchSearchFindMany.mockResolvedValue([
      batchSearchDispatch(
        searchRef,
        {
          id: "re-stamped-local-reader-probe",
          outcome: "NO_MATCH",
          observedAt,
          runtimeVersion: releaseSha,
        },
        observedAt,
        "LOCAL_BROWSER_READER",
        new Date("2026-07-15T19:51:59.999Z"),
      ),
    ]);
  ...
  it("accepts a source-newer success ahead of a delayed-written older local-reader failure during closeout", async () => {
    const { closeoutAt, successProbe, batch } =
      prepareExactImplementationLocalReaderCloseout(
  ...
  it("fails closed when the source-time winner changes during the transactional closeout reread", async () => {
    const { closeoutAt, successProbe, batch } =
      prepareExactImplementationLocalReaderCloseout(
        "2026-07-15T20:29:45.000Z",
      );
  ...
  it("consumes a started deferred confirmation that still observes F1 without replaying F2", async () => {
    const fixture = deferredFailureCarrierBatch({
      observedFailureClass: "HTTP_5XX",
      providerExecution: true,
      started: true,
  ...
  it("consumes a started deferred confirmation without provider execution but keeps canonical F1", async () => {
    const fixture = deferredFailureCarrierBatch({
      observedFailureClass: "MISSING_SOURCE",
      providerExecution: false,
      started: true,
  ...
  it("keeps mixed restored and unfinished structural work automatic", async () => {
    ...
    const mixedRequestUpdatedAt = new Date(now.getTime() - 20_000);
    const mixedPreExecutionRequest = detachedRequestState("STALE", {
      id: "request-entry-2",
      ...
      providerSnapshotFingerprint: providerFingerprint,
      nextAttemptAt: null,
      completedAt: mixedRequestUpdatedAt,
      createdAt: new Date(now.getTime() - 120_000),
      updatedAt: mixedRequestUpdatedAt,
    });
    prismaMocks.batchFindFirst.mockResolvedValue(batch);
    prismaMocks.verificationRequestFindMany.mockResolvedValue([
      mixedPreExecutionRequest,
    ]);
    prismaMocks.verificationRequestUpdateMany.mockResolvedValue({ count: 1 });
    ...
    expect(prismaMocks.verificationRequestUpdateMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: expect.objectContaining({
          id: mixedPreExecutionRequest.id,
          revision: mixedPreExecutionRequest.revision,
          startedAt: mixedPreExecutionRequest.startedAt,
          updatedAt: mixedRequestUpdatedAt,
        }),
        data: {
          revision: { increment: 0 },
  ...
  it("reschedules an assigned adapter when endpoint work never reaches provider execution", async () => {
    ...
    prismaMocks.batchFindFirst.mockResolvedValue(batch);
    const endpointRequestUpdatedAt = new Date(now.getTime() - 20_000);
    const endpointPreExecutionRequest = detachedRequestState("STALE", {
      id: "request-endpoint-pre-execution",
      batchIncidentId: "entry-1",
      releaseSha,
      runtimeVersion: releaseSha,
      revision: 2,
      ...
      providerSnapshotFingerprint: providerFingerprint,
      nextAttemptAt: null,
      completedAt: endpointRequestUpdatedAt,
      createdAt: new Date(now.getTime() - 120_000),
      updatedAt: endpointRequestUpdatedAt,
    });
    prismaMocks.verificationRequestFindMany.mockResolvedValue([
      endpointPreExecutionRequest,
    ]);
    prismaMocks.verificationRequestUpdateMany.mockResolvedValue({ count: 1 });
    prismaMocks.batchUpdateMany.mockResolvedValue({ count: 1 });
    prismaMocks.supportIncidentUpdateMany.mockResolvedValue({ count: 1 });
    ...
    expect(prismaMocks.verificationRequestUpdateMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: expect.objectContaining({
          id: endpointPreExecutionRequest.id,
          revision: endpointPreExecutionRequest.revision,
          startedAt: endpointPreExecutionRequest.startedAt,
          evidence: { equals: expect.anything() },
          updatedAt: endpointRequestUpdatedAt,
        }),
        data: {
  ...
  it("does not count PRE_EXECUTION on an earlier release as provider execution", async () => {
    const batch = closeoutBatch("PENDING");
    const exactFailureFingerprint = "c".repeat(64);
    ...
    const requestCreatedAt = new Date(now.getTime() - 120_000);
    const earlierRequestUpdatedAt = new Date(now.getTime() - 45_000);
    const currentRequestUpdatedAt = new Date(now.getTime() - 90_000);
    const earlierStartedRequest = detachedRequestState("RETRYABLE_FAILED", {
      ...
      updatedAt: earlierRequestUpdatedAt,
    });
    const currentNeverStartedRequest = detachedRequestState("STALE", {
      ...
      lastError: null,
      createdAt: requestCreatedAt,
      updatedAt: currentRequestUpdatedAt,
    });
    prismaMocks.batchFindFirst.mockResolvedValue(batch);
    prismaMocks.verificationRequestFindMany.mockResolvedValue([
    ...
    expect(prismaMocks.verificationRequestUpdateMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: expect.objectContaining({
          id: "request-earlier-release",
          revision: 4,
          startedAt: earlierStartedRequest.startedAt,
          evidence: { equals: expect.anything() },
          updatedAt: earlierRequestUpdatedAt,
        }),
        data: {
  ...
  it("aborts closeout when a terminal request becomes CHECKING in the serializable reread", async () => {
    const batch = closeoutBatch("PENDING");
    batch.baseSha = "d".repeat(40);
    batch.releaseSha = null;
    batch.deployedAt = null;
    batch.summary = operationalRemediationSummary();
    const terminalUpdatedAt = new Date(now.getTime() - 60_000);
    const terminalRequest = detachedRequestState("STALE", {
      id: "request-terminal-to-checking",
      batchIncidentId: "entry-1",
      releaseSha: batch.baseSha,
      runtimeVersion: batch.baseSha,
      revision: 3,
      attemptCount: 1,
      startedAt: new Date(terminalUpdatedAt.getTime() - 30_000),
      outcome: null,
      failureClass: null,
      evidence: null,
      lastError: "pre_execution_stopped",
      ...
      nextAttemptAt: null,
      completedAt: terminalUpdatedAt,
      createdAt: new Date(terminalUpdatedAt.getTime() - 120_000),
      updatedAt: terminalUpdatedAt,
    });
    const checkingUpdatedAt = new Date(now.getTime() - 10_000);
    const checkingRequest = {
      ...terminalRequest,
      status: "CHECKING" as const,
      revision: 4,
      lastError: null,
      completedAt: null,
      updatedAt: checkingUpdatedAt,
    };
    prismaMocks.batchFindFirst.mockResolvedValue(batch);
    prismaMocks.verificationRequestFindMany
      .mockResolvedValueOnce([terminalRequest])
      .mockResolvedValueOnce([checkingRequest])
      .mockResolvedValue([checkingRequest]);
    prismaMocks.transaction.mockImplementation(
      async (
        worker: (
          transaction: typeof monitoringTransactionClient,
  ...
  it("resnapshots closeout when an earlier-release request starts inside the transaction", async () => {
    ...
    const requestCreatedAt = new Date(now.getTime() - 120_000);
    const oldRequestUpdatedAt = new Date(now.getTime() - 90_000);
    const currentRequestUpdatedAt = new Date(now.getTime() - 80_000);
    const freshRequestUpdatedAt = new Date(now.getTime() - 20_000);
    ...
    const freshRequests = [
      {
        ...
        revision: 3,
        startedAt: new Date(now.getTime() - 30_000),
        updatedAt: freshRequestUpdatedAt,
      },
      currentRequest,
    ];
    prismaMocks.batchFindFirst.mockResolvedValue(batch);
    ...
    expect(prismaMocks.verificationRequestUpdateMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: expect.objectContaining({
          id: "request-old-release",
          revision: 3,
          startedAt: freshRequests[0].startedAt,
          updatedAt: freshRequestUpdatedAt,
        }),
        data: {
          revision: { increment: 0 },
  ...
  it.each([
    ...
    async (scenario) => {
      ...
      const requestCreatedAt = new Date(now.getTime() - 120_000);
      const neverStartedUpdatedAt = new Date(now.getTime() - 90_000);
      const startedUpdatedAt = new Date(now.getTime() - 20_000);
      const neverStartedRequest = {
        ...detachedRequestState("STALE", {
          ...
          lastError: null,
          createdAt: requestCreatedAt,
          updatedAt: neverStartedUpdatedAt,
        }),
      };
      ...
      expect(prismaMocks.verificationRequestUpdateMany).toHaveBeenCalledWith(
        expect.objectContaining({
          where: expect.objectContaining({
            ...
            revision: startedRequest.revision,
            startedAt: startedRequest.startedAt,
            updatedAt: startedUpdatedAt,
          }),
          data: {
            revision: { increment: 0 },
  ...
  it("audits blocked environment closeout without consuming the planned remediation", async () => {
    ...
    batch.summary = {
      ...healthyDetachedDispatch(),
      searchExecutionFence: emptySearchExecutionFence(),
      plannedPaths: ["docs/course-support-provider-notes/example.md"],
      remediation: {
        attempts: [
          {
            courseRef: createHash("sha256").update("course-1").digest("hex").slice(0, 24),
  ...
  it("parks the second unchanged operational closeout without scheduling another AI retry", async () => {
    ...
    batch.incidents[0].proofSnapshot = {
      ...
      outcome: "FETCH_FAILED",
      observedAt: new Date(now.getTime() - 30_000).toISOString(),
      // A durable provider attempt on an earlier runtime consumes the bounded
      // operational no-progress budget without counting as current proof.
      runtimeVersion: "b".repeat(40),
      providerExecution: true,
    };
    ...
    expect(
      countCourseSupportOperationalNoProgressAttempts({
        batchIncidents: batch.incidents[0].incident.batchIncidents,
        cycle: 1,
        courseId: "course-1",
        providerSnapshotFingerprint: providerFingerprint,
        failureFingerprint: "fingerprint",
  ...
  it("consumes a deployed implementation that later fails production verification", async () => {
    ...
    const branch = "automation/course-support-example";
    const runtimePath = "src/lib/tee-times/adapters/example.ts";
    batch.baseSha = baseSha;
    batch.incidents[0].incident.escalationDeadlineAt = new Date(now.getTime() + 10 * 60_000);
    batch.summary = {
      ...healthyDetachedDispatch(),
      branch,
      plannedPaths: [runtimePath],
      releaseProvenance: {
        schemaVersion: 1,
        fromSha: baseSha,
        toSha: releaseSha,
  ...
  it.each([
    {
      ...
      expectedActionState: "NOT_EXECUTED",
      expectedActionReason: "CURRENT_RUNTIME_PROOF_MISSING"
    },
    {
      label: "consumes current provider execution proof",
      proofRuntimeVersion: "c".repeat(40),
      expectedConsumed: true,
      expectedActionState: "EXECUTED",
      expectedActionReason: "CURRENT_RUNTIME_PROOF_RECORDED"
    }
    ...
    expect(persistedAttempt).toMatchObject({
      ...
      actionExecution: {
        schemaVersion: 1,
        action: "VERIFY_CURRENT_RUNTIME",
        state: testCase.expectedActionState,
        reason: testCase.expectedActionReason
      }
    });
    expect(persistedAttempt?.orchestrationRetry).toBeNull();
  });
  ...
  it("keeps a LOCAL_READER t1 result eligible when closeout t2 precedes consumption t3", async () => {
    const priorProviderEvidenceAt = new Date("2026-07-15T19:45:00.000Z");
    const localReaderStartedAt = now;
    const localReaderProviderObservedAt = new Date(
      "2026-07-15T20:00:30.000Z",
    );
    const closeoutAt = new Date("2026-07-15T20:01:00.000Z");
    const localReaderConsumedAt = new Date("2026-07-15T20:02:00.000Z");
    const continuation = zeroExecutionContinuation();
    const batch = sameIdentityZeroExecutionRetryBatch(continuation.proof);
    batch.incidents[0].incident.lastSeenAt = priorProviderEvidenceAt;
    batch.incidents[0].incident.attemptLedger =
      localReaderStartedAttemptLedger(1, localReaderStartedAt);
    prismaMocks.batchFindFirst.mockResolvedValue(batch);
    prismaMocks.verificationRequestFindMany.mockResolvedValue([
      continuation.request,
    ]);
    ...
    expect(incidentUpdate?.data.lastSeenAt).toEqual(priorProviderEvidenceAt);
    expect(localReaderStartedAt.getTime()).toBeLessThan(
      localReaderProviderObservedAt.getTime(),
    );
    expect(localReaderProviderObservedAt.getTime()).toBeLessThan(
      closeoutAt.getTime(),
    );
    expect(closeoutAt.getTime()).toBeLessThan(
      localReaderConsumedAt.getTime(),
    );
    expect(localReaderProviderObservedAt.getTime()).toBeGreaterThan(
      incidentUpdate?.data.lastSeenAt.getTime() ?? Number.POSITIVE_INFINITY,
    );
  });
  ...
  it("allows retry closeout after verify copied the exact current detached failure", async () => {
    ...
    expect(prismaMocks.supportIncidentUpdateMany).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ lastSeenAt: observedAt }),
      }),
    );
  });

  it("persists provider source time for a NEEDS_HUMAN closeout", async () => {
    const batch = closeoutBatch("NEEDS_HUMAN", detachedFailureProof());
    Object.assign(batch.incidents[0].incident, {
      kind: "FETCH_FAILED",
      failureClass: "RATE_LIMIT",
      failureFingerprint: buildProviderFailureFingerprint({
