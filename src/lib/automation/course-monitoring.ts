...
const GENERIC_COURSE_NEARBY_IDENTITY_REVALIDATION = "generic-course-exact-place-source-search-v3";
const PRIOR_GENERIC_COURSE_NEARBY_IDENTITY_REVALIDATION = "generic-course-nearby-source-search-v2";
  ...
  for (const candidate of eligible) {
    const changed = await runSerializedCourseMonitoringWrite(candidate.courseId, async transaction => {
      ...
      const updated = await transaction.courseSupportIncident.updateMany({where: {
        ...
        latestMessage: "An exact-place and nearby official course identity check is available; discovery is queued.",
      }});
      ...
      await transaction.courseMonitoringEvent.create({data: {
        ...
        message: "Exact public place, nearby course, and first-party page corroboration can now be attempted.",
