...
import {
  ...
  assessParkedCourseIncompleteStageContinuation,
  hasConsumedIncompletePlaybookContinuation,
...
function hasNewerOrEqualFinalClassificationProviderSource(
  ...
  return Boolean(
    (snapshot.providerObservation &&
      snapshot.providerObservation.observationStartedAt >=
        evidenceObservedAt) ||
    snapshot.localReaderObservations.some(
      (observation) => observation.providerObservedAt >= evidenceObservedAt,
    ),
  );
}
...
function finalCourseIntelligenceUpdateMatchesCurrent(
  ...
  return (
    ...
    datesMatch(update.intelligenceVerifiedAt, course.intelligenceVerifiedAt) &&
  ...
  return runSerializedCourseMonitoringWrite(
    ...
    async (transaction) => {
      ...
      const courseHasNewerIntelligence = Boolean(
        ...
        course.intelligenceVerifiedAt.getTime() > evidenceObservedAt.getTime(),
      );
...
export async function invalidateReviewDerivedIdentityFinal(input: {
  ...
  if (
    ...
    throw new Error(
      "Review-final invalidation requires a valid prior evidence time.",
    );
  }
  if (
...
export function canRevalidateGenericCourseNearbyIdentity(incident: GenericIdentityRevalidationIncident) {
  ...
  const sourceUnverifiedFinal = incident.status === "RESOLVED" &&
    incident.resolution === "SOURCE_UNVERIFIED" && incident.resolvedAt !== null &&
    incident.decisionAt === null && course?.providerFamilyKey === "SOURCE_MISSING" &&
    !course.website && !course.detectedBookingUrl;
  const expectedState = sourceUnverifiedFinal ? "FINAL_TECHNICAL" : incident.status === "NEEDS_HUMAN"
  ...
  if (!course || !expectedState || incident.activeBatchId !== null ||
      incident.decisionAt !== null ||
      (!sourceUnverifiedFinal && (incident.resolvedAt !== null || incident.resolution !== null)) ||
...
async function revalidateGenericCourseNearbyIdentityForDeployment(deploymentSha: string) {
  const candidates = await prisma.courseSupportIncident.findMany({
    where: {status: {in: ["NEEDS_HUMAN", "AUTO_INVESTIGATING", "RESOLVED"]}, activeBatchId: null, decisionAt: null,
      OR: [{resolvedAt: null, resolution: null},
        {status: "RESOLVED", resolution: "SOURCE_UNVERIFIED", resolvedAt: {not: null}}],
      ...
      monitoringEvents: {
        some: {eventType: "REVALIDATION_REQUESTED",
          readPath: PRIOR_GENERIC_COURSE_NEARBY_IDENTITY_REVALIDATION},
  ...
  for (const candidate of eligible) {
    const changed = await runSerializedCourseMonitoringWrite(candidate.courseId, async transaction => {
      ...
      const updated = await transaction.courseSupportIncident.updateMany({where: {
        ...
        activeBatchId: null, decisionAt: null, resolvedAt: incident.resolvedAt,
        resolution: incident.resolution,
  ...
  if (
    ...
    (failureRefinementIncompleteRecovery &&
      (admissionAudit.admissionMode !==
        ...
        typeof admissionAudit.failureRefinementLineageDigest !== "string" ||
        !/^[a-f0-9]{64}$/u.test(
          admissionAudit.failureRefinementLineageDigest,
        ) ||
        !Number.isInteger(admissionAudit.failureRefinementEventCount) ||
        Number(admissionAudit.failureRefinementEventCount) < 1 ||
...
export async function reopenParkedCourseForResponderCampaign(
  ...
  return runSerializedCourseMonitoringWrite(
    ...
    input.admissionMode ===
      ...
      input.admissionMode === "UNFINISHED_STAGE_CONTINUATION"
      ? { isolationLevel: Prisma.TransactionIsolationLevel.Serializable }
      : undefined,
  );
}
...
export async function reopenParkedCourseForResponderCampaignInTransaction(
  ...
  const startedLocalReaderContinuationRequested =
    input.admissionMode === "STARTED_LOCAL_READER_CONTINUATION";
  const stageContinuationRequested = input.admissionMode === "UNFINISHED_STAGE_CONTINUATION";
  const sameIdentityMaterialChangeIncompleteRecoveryRequested =
    input.admissionMode ===
    "SAME_IDENTITY_MATERIAL_CHANGE_INCOMPLETE_PLAYBOOK_RECOVERY";
  const failureRefinementIncompletePlaybookRecoveryRequested =
    input.admissionMode === "FAILURE_REFINEMENT_INCOMPLETE_PLAYBOOK_RECOVERY";
  const exactRuntimeSourceCycleRecoveryRequested =
    input.admissionMode === "EXACT_RUNTIME_SOURCE_CYCLE_RECOVERY";
  const postMarkerIncompletePlaybookRecoveryRequested =
    input.admissionMode === "POST_MARKER_INCOMPLETE_PLAYBOOK_RECOVERY";
  const exactEvidenceRecoveryRequested =
    stageContinuationRequested ||
    input.admissionMode === "DESCENDANT_INCOMPLETE_PLAYBOOK_RECOVERY" ||
  ...
  const campaignCapturedAt = input.campaignCapturedAt
    ...
    : null;
  const completeCampaignHistorySince =
  ...
  return (async () => {
    const incident = await transaction.courseSupportIncident.findUnique({
      ...
      select: {
        ...
        decisionIdempotencyKey: true,
        monitoringEvents: {
          ...(completeCampaignHistorySince
            ? {
                where: {
                  occurredAt: { gte: completeCampaignHistorySince },
                },
              }
            : { take: 50 }),
          orderBy: [{ occurredAt: "desc" }, { id: "desc" }],
            ...
            select: {
              ...
              batch: {
                select: {
                  ...
                  revision: true,
                  ownerAutomationRunId: true,
                  ...(startedLocalReaderContinuationRequested || stageContinuationRequested
                    ? { _count: { select: { incidents: true } } }
                    : {}),
                  baseSha: true,
                  releaseSha: true,
    ...
    const currentCampaignMember =
      incident && status && currentPlaybookAssessment
        ? {
            ...
            zeroExecutionEvidence: {
              ...
              latestDiscoveryTimestampRowCount: incident.course
                ...
                : 0,
              monitoringHistoryCompleteSince: completeCampaignHistorySince,
        ...
        : null;
    const sameCycleRecoveryAction = stageContinuationRequested
      ? "parked_cohort_unfinished_stage_continuation"
      : startedLocalReaderContinuationRequested
            ...
            : requestlessStaleOwnershipRecoveryRequested
              ...
              : sameIdentityMaterialChangeIncompleteRecoveryRequested
                ...
                : currentCycleOrchestrationRecoveryRequested
                  ...
                  : null;
    const sameCycleRecoveryAlreadyRecorded = stageContinuationRequested && currentPlaybookAssessment
      ? hasConsumedIncompletePlaybookContinuation({
          events: incident?.monitoringEvents ?? [],
          campaignRunId: input.campaignRunId,
          cycle: input.expectedCycle,
          stage: currentPlaybookAssessment.nextStage ?? "",
          attemptCount: currentPlaybookAssessment.stages.find(stage => stage.stage === currentPlaybookAssessment.nextStage)?.attemptCount ?? -1,
        })
      : Boolean(
    ...
    const startedLocalReaderContinuation =
      ...
      readerReadiness
        ...
        : null;
    const incompletePlaybookRecovery = stageContinuationRequested && currentCampaignMember &&
      descendantCampaignRun?.promptVersion === PARKED_COURSE_CAMPAIGN_PROMPT_VERSION &&
      descendantCampaignRun.status === "RUNNING" && descendantCampaignRun.completedAt === null &&
      descendantCampaignAudit?.membershipDigest === input.campaignMembershipDigest &&
      descendantCampaignAudit.capturedAt === input.campaignCapturedAt &&
      descendantCapturedMember && campaignCapturedAt &&
      descendantCapturedMember.revision === input.capturedRevision &&
      descendantCapturedMember.monitoringRevision === input.capturedMonitoringRevision &&
      descendantCapturedMember.cycle === input.capturedCycle &&
      descendantCapturedMember.kind === input.capturedKind &&
      descendantCapturedMember.providerFamilyKey === input.capturedProviderFamilyKey
      ? assessParkedCourseIncompleteStageContinuation({
          captured: descendantCapturedMember, current: currentCampaignMember,
          capturedAt: campaignCapturedAt, campaignRunId: input.campaignRunId,
          campaignMembershipDigest: input.campaignMembershipDigest,
        }) : null;
    const sameCycleRecoveryHistory =
      sameCycleRecoveryRequested && incident
        ? stageContinuationRequested
          ? incompletePlaybookRecovery
          : startedLocalReaderContinuationRequested
    ...
    if (
      ...
      (!descendantCampaignRun ||
        ...
        input.expectedPlaybookNextStage !== null ||
        ...
        automationStalledEndpoint.id !==
          exactRuntimeSourceCycleRecovery.supersededEndpointId ||
        automationStalledEndpoint.occurredAt.getTime() !==
          exactRuntimeSourceCycleRecovery.supersededEndpointAt.getTime())
    ) {
        ...
        (!Number.isInteger(input.capturedCycle) ||
          ...
          (stageContinuationRequested &&
            (!incompletePlaybookRecovery ||
              input.expectedLatestProbeId === undefined ||
              input.expectedLatestDiscoveryId === undefined ||
              (incident.course.probes[0]?.id ?? null) !== input.expectedLatestProbeId ||
              (incident.course.automationDiscoveries[0]?.id ?? null) !== input.expectedLatestDiscoveryId)) ||
          (incompletePlaybookRecoveryRequested &&
            (input.expectedCycle !== (input.capturedCycle ?? 0) + 1 ||
              !originalAdmission || automationStalledEndpoint.occurredAt < originalAdmission.occurredAt ||
          ...
          (failureRefinementIncompletePlaybookRecoveryRequested &&
            (!descendantCampaignRun ||
              ...
              failureRefinementIncompletePlaybookRecovery.history
                .historyDigest !==
                input.expectedSameCycleRecoveryHistoryDigest ||
              failureRefinementIncompletePlaybookRecovery.history
                .startedRequestCount < 1 ||
              automationStalledEndpoint.occurredAt <
                failureRefinementIncompletePlaybookRecovery.refinementAt)) ||
          (postMarkerIncompletePlaybookRecoveryRequested &&
            (!descendantCampaignRun ||
              descendantCampaignRun.promptVersion !==
                PARKED_COURSE_CAMPAIGN_PROMPT_VERSION ||
    ...
    if (exactRuntimeSourceCycleRecoveryRequested) {
      ...
      const currentCycleRequestIds =
        exactRuntimeSourceCycleRecovery!.history.requestFences.map(
          (request) => request.id,
        );
      ...
      if (
        ...
        (latestProbe?.observedAt.toISOString() ?? null) !==
          input.expectedLatestProbeAt ||
        (latestDiscovery?.id ?? null) !== input.expectedLatestDiscoveryId ||
    ...
    if (sameCycleRecoveryRequested) {
      ...
      const idempotencyKey = startedLocalReaderContinuationRequested
        ...
        : stageContinuationRequested
          ? `course-support-incomplete-stage:${input.campaignRunId}:${incident.id}:${incident.cycle}:${currentPlaybookAssessment!.nextStage}:${currentPlaybookAssessment!.stages.find(stage => stage.stage === currentPlaybookAssessment!.nextStage)!.attemptCount}`
      ...
      const incidentUpdated =
        await transaction.courseSupportIncident.updateMany({
          ...
          data: {
            ...
            nextReminderAt: null,
            nextAttemptAt: now,
            nextAction: stageContinuationRequested
              ? "Continue the unfinished stage once without resetting the current cycle or retained attempts."
              : startedLocalReaderContinuationRequested
      ...
      await transaction.courseMonitoringEvent.create({
        data: {
          ...
          audit: {
            ...
            ...(stageContinuationRequested ? {
              playbookStageAttemptCount: currentPlaybookAssessment!.stages.find(stage => stage.stage === currentPlaybookAssessment!.nextStage)!.attemptCount,
            } : {}),
            campaignRunId: input.campaignRunId,
            campaignMembershipDigest: input.campaignMembershipDigest,
            ...
            failureRefinementLineageDigest:
              failureRefinementIncompletePlaybookRecovery?.lineageDigest ??
              null,
            failureRefinementEventCount:
              failureRefinementIncompletePlaybookRecovery?.refinementEventCount ??
              null,
    ...
    const nextCycle = incident.cycle + 1;
    const idempotencyKey = exactRuntimeSourceCycleRecoveryRequested
      ? `course-support-parked-cohort-exact-runtime-source-cycle:${input.campaignRunId}:${incident.id}:${nextCycle}:${input.currentRuntimeVersion}`
      : `course-support-parked-cohort:${input.campaignRunId}:${incident.id}:${nextCycle}`;
    const incidentUpdated = await transaction.courseSupportIncident.updateMany({
      where: {
        id: incident.id,
        courseId: input.courseId,
      ...
      data: {
        ...
        nextReminderAt: null,
        nextAttemptAt: now,
        nextAction: exactRuntimeSourceCycleRecoveryRequested
    ...
    await transaction.courseMonitoringEvent.create({
      data: {
        ...
        toState: "AUTO_INVESTIGATING",
        failureFingerprint: input.expectedFailureFingerprint,
        message: exactRuntimeSourceCycleRecoveryRequested
          ? "The responder opened one fresh campaign cycle so every source and reader stage can execute on one exact deployed runtime."
          : "The responder admitted one parked campaign member to a fresh ordered-playbook cycle.",
        idempotencyKey,
        occurredAt: now,
        audit: exactRuntimeSourceCycleRecoveryRequested
          ? {
              ...
              exactRuntimeLineageDigest:
                exactRuntimeSourceCycleRecovery!.lineageDigest,
              sameCycleRecoveryHistoryDigest:
                input.expectedSameCycleRecoveryHistoryDigest,
              sourceRuntimeVersion:
                exactRuntimeSourceCycleRecovery!.sourceRuntimeVersion,
              recoveryRuntimeVersion: input.currentRuntimeVersion,
              supersededEndpointId:
                exactRuntimeSourceCycleRecovery!.supersededEndpointId,
              supersededEndpointAt:
                exactRuntimeSourceCycleRecovery!.supersededEndpointAt.toISOString(),
              providerSnapshotFingerprint:
                input.expectedProviderSnapshotFingerprint,
              attemptLedgerFingerprint: input.expectedAttemptLedgerFingerprint,
              latestProbeAt: input.expectedLatestProbeAt,
