...
function learnOfficialPhoneReservationClassification(
  ...
  if (
    ...
    !hasStrongCourseIdentityEvidence(evidence.courseName, visibleText, directPhone)
  ) {
    return null;
  }
...
function findDirectTeeTimeReservationPhone(visibleText: string): DirectReservationPhone {
  const patterns = [
    /\b(?:please\s+)?call\s*:?\s*(?:(?:the\s+)?pro\s+shop\s*)?(?:at\s*)?((?:\+?1[\s.-]*)?(?:\(\s*\d{3}\s*\)|\d{3})[\s.-]*\d{3}[\s.-]*\d{4})\s*(?:,|\s)*(?:to|for)\s+(?:book|reserve|schedule|make)\s+(?:a\s+|your\s+|the\s+)?(?:tee\s*times?|tee\s*time\s+reservations?)/gi,
