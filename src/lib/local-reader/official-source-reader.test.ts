...
function fixture(row = cases[0]) {
  const course = { id: "course", name: row.name, address: `${row.street}, Omaha, NE`, city: "Omaha", stateCode: "NE", timeZone: "America/Chicago", website: row.website };
...
function read(row: typeof cases[number], course: OfficialSourceJob["course"], extra = "") {
  document.body.innerHTML = `<h1>${row.title}</h1><p>${row.street}</p><footer>Official Site Of City Of Omaha, NE Parks Department</footer>
  ...
  it.each(cases)("resolves $name including its historical visible selector", async row => {
    ...
    if (row === cases[1]) {
      document.body.innerHTML = `<h2>Omaha's Premier Golf Courses</h2>
        <a href="/elmwoodgolf-course/">Elmwoodgolf Course</a>
    ...
    const fetchImpl = vi.fn<typeof fetch>().mockResolvedValue(Response.json([{ id: row.facility,
      name: row.title, address: job.course.address, timeZone: job.course.timeZone }]));
