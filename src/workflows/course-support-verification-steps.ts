...
async function runWithCourseSupportProviderObservation<T>(input: {
  courseId: string;
  worker: (context: {
    ...
    if (providerExecutionStarted) {
      ...
      if (!retained && !heartbeatError) {
        heartbeatError = new Error(
          "Provider verification source could not be retained for reconciliation.",
        );
      }
      ...
      if (!retained && !heartbeatError) {
        heartbeatError = new Error(
          "Superseded provider verification source could not be retained.",
        );
      }
    } else {
...
export async function executeCourseSupportVerificationStep(
  ...
  const handleLocalReaderVerification = async () => {
    ...
    if (readerVerification?.status === "TERMINAL") {
      const providerExecution = readerVerification.resultStatus !== "EXPIRED";
      return failVerification({
        input,
        revision,
        runtimeVersion,
        ...
        providerExecution,
        ...(providerExecution
          ? { providerObservedAt: readerVerification.observedAt }
          : {}),
        message:
          readerVerification.resultStatus === "ACCESS_CHALLENGE"
  ...
  const observation = await runWithCourseSupportProviderObservation({
    courseId,
    worker: async ({
      providerObservedAt,
      markProviderExecutionStarted,
      ...
      try {
        ...
        if (!providerReadError) throw error;
        const failure = classifyProviderFailure({ error: providerReadError });
      ...
      if (!execution.acquired) {
        ...
        return {
          ...
          result: await failVerification({
            ...
            providerExecution: false,
            message:
              "Provider verification was deferred by the concurrency guard.",
            retryAt: new Date(Date.now() + LEASE_BUSY_RETRY_MS),
          }),
        };
      }
  ...
  if (!observation.acquired) {
    return failVerification({
      ...
      failureClass: "RATE_LIMIT",
      providerExecution: false,
      message: "Another provider observation is already in progress.",
      retryAt: new Date(Date.now() + LEASE_BUSY_RETRY_MS),
    });
  }
  if (observation.value.kind === "result") return observation.value.result;

  const readerResult = await handleLocalReaderVerification();
  if (readerResult) return readerResult;
  const { failure, failedAt, providerObservedAt } = observation.value;
  return failVerification({
    input,
    revision,
    runtimeVersion,
    ...
    retryAfterSeconds: failure.retryAfterSeconds,
    message: "Public provider availability verification failed.",
    retryAt: new Date(failedAt.getTime() + LEASE_BUSY_RETRY_MS),
  });
}
...
async function executeDeferredFailureConfirmation(input: {
  ...
  if (observation.acquired) return observation.value;
  return failVerification({
    input: input.input,
    revision,
    runtimeVersion: input.runtimeVersion,
...
> {
  ...
  const observation = await runWithCourseSupportProviderObservation({
    ...
    worker: async ({
      ...
      if (!failure && execution && !execution.acquired) {
        failure = {
          failureClass: "RATE_LIMIT",
          httpStatus: null,
      ...
      if (!failure && execution?.acquired) {
        ...
        const outcome: "MATCH_FOUND" | "NO_MATCH" =
          matchingSlots.length > 0 ? "MATCH_FOUND" : "NO_MATCH";
        assertObservationOwned();
        const verified = await markCourseSupportVerificationDiscoveryVerified({
          requestId: input.input.requestId,
          expectedRevision: revision,
        ...
        if (!verified.marked) {
          return {
            ...
            value: { outcome: "stopped" as const, reason: verified.reason },
          };
        }
        revision = verified.revision;
        discoveryAttemptedAt = verified.discoveryAttemptedAt;
...
async function failVerification(input: {
  ...
  const failed = await failCourseSupportVerificationRequest({
    ...
    observation: {
      ...
      observedAt: input.providerObservedAt ?? new Date(),
