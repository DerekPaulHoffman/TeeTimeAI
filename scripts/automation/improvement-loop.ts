...
- When inspection returns courseProfileQueue items and no higher-priority incident is active, process up to three as one metadata_seo batch. Use automation:course-profile research packets, source-backed original prose, dry-run validation, and apply only profiles that pass; never delay or disable alert creation when evidence is insufficient.
